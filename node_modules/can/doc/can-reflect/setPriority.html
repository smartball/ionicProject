...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-reflect/reflections/observe/observe.js
	@function can-reflect/setPriority
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - setPriority</title>
...
<div class="breadcrumb">
	...
	<li><a href="setPriority.html">setPriority</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>setPriority</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-reflect/edit/master/reflections/observe/observe.js#L441">Edit on GitHub</a>
				</li>
    ...
    <p>Provide a priority for when an observable that derives its value should be re-evaluated.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>setPriority(obj, priority)</code>
  </h2>
  <p>Calls an underlying <code>@can.setPriority</code> symbol on <code>obj</code> if it exists with <code>priorty</code>.
Returns <code>true</code> if a priority was set, <code>false</code> if otherwise.</p>
<p>Lower priorities (<code>0</code> being the lowest), will be an indication to run earlier than
higher priorities.</p>
<pre><code class="language-js">var obj = canReflect.assignSymbols({},{
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>obj</b> <code>{Object}</code>: <p>An observable that will update its priority.</p>
</li>
        
          <li><b>priority</b> <code>{Number}</code>: <p>The priority number.  Lower priorities (<code>0</code> being the lowest),
indicate to run earlier than higher priorities.</p>
</li>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{Boolean}</code>: <p><code>true</code> if a priority was able to be set, <code>false</code> if otherwise.</p>
</p>
  </div>
    ...
    <section class="body">
    <h2>Use</h2>
<p>There's often a need to specify the order of re-evaluation for
<strong>observables</strong> that derive (or compute) their value from other observables.</p>
<p>This is needed by templates to avoid unnecessary re-evaluation.  Say we had the following template:</p>
<pre><code class="language-js">{{#if value}}
  {{value}}
{{/if}}
</code></pre>
<p>If <code>value</code> became falsey, we'd want the <code>{{#if}}</code> to be aware of it before
the <code>{{value}}</code> magic tags updated. We can do that by setting priorities:</p>
<pre><code class="language-js">canReflect.setPriority(magicIfObservable, 0);
canReflect.setPriority(magicValueObservable,1);
</code></pre>
<p>Internally, those observables will use that <code>priority</code> to register their
re-evaluation with the <code>derive</code> queue in [can-queues].</p>
...
<script type="text/javascript">
  window.docObject = {"type":"function","name":"can-reflect/setPriority","parent":"can-reflect/observe","src":{"line":440,"codeLine":499,"path":"node_modules/can-reflect/reflections/observe/observe.js"},"description":"Provide a priority for when an observable that derives its value should be re-evaluated.\n\n","title":"setPriority","signatures":[{"code":"setPriority(obj, priority)","description":"\n\nCalls an underlying `@can.setPriority` symbol on `obj` if it exists with `priorty`.\nReturns `true` if a priority was set, `false` if otherwise.\n\nLower priorities (`0` being the lowest), will be an indication to run earlier than\nhigher priorities.\n\n```js\nvar obj = canReflect.assignSymbols({},{\n  \"can.setPriority\": function(priority){\n    return this.priority = priority;\n  }\n});\n\ncanReflect.setPriority(obj, 0) //-> true\nobj.priority //-> 0\n\ncanReflect.setPriority({},20) //-> false\n```\n","params":[{"types":[{"type":"Object","options":[]}],"name":"obj","description":"An observable that will update its priority."},{"types":[{"type":"Number"}],"name":"priority","description":"The priority number.  Lower priorities (`0` being the lowest),\nindicate to run earlier than higher priorities."}],"returns":{"types":[{"type":"Boolean"}],"description":"`true` if a priority was able to be set, `false` if otherwise.\n"}}],"_curParam":{"types":[{"type":"Number"}],"name":"priority","description":"The priority number.  Lower priorities (`0` being the lowest),\nindicate to run earlier than higher priorities."},"_curReturn":{"types":[{"type":"Boolean"}],"description":"`true` if a priority was able to be set, `false` if otherwise.\n"},"comment":" ","pathToRoot":"../.."};
</script>
</article>
