...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-zone/docs/can-zone.md
	@module can-zone
######################################################################## -->
<html lang="en">
<head>
...
<div class="breadcrumb">
  ...
  <section class="title">
			...
			<ul class="title-social">
				...
				<li>
					<a class="npm-button" href="https://www.npmjs.com/package/can-zone">
						<img src="https://img.shields.io/badge/npm%20package-0.6.13-brightgreen.svg" alt="npm package badge" />
					</a>
				</li>
				<li>
					<a class="github-button nav-social" href="https://github.com/canjs/can-zone" data-show-count="true">Star</a>
				</li>
				
				
				<li>
					<a class="button-link" href="//github.com/canjs/can-zone/edit/master/docs/can-zone.md">Edit on GitHub</a>
				</li>
    ...
    <p>A library that tracks asynchronous activity and lets you know when it has completed. Useful when you need to call a function and wait for all async behavior to complete, such as when performing server-side rendering.</p>
...
<p>*Note: See the &lt;a href=&quot;can-zone/register.html&quot; title=&quot;In order to do it's magic, can-zone has to register handlers for all of the common JavaScript async operations. If you have code (or a dependency with this code) that does:
var st = setTimeout;</p>
<p>And this module loads before can-zone, any time st is used we won't be able to track that within the Zone.
To work around this, can-zone/register is used as a script that you run before any other modules.
In Node
require(&quot;can-zone/register&quot;);</p>
<p>At the top of your entry-point script.
In the Browser
You can either add a script tag above all others:
&lt;script src=&quot;node_modules/can-zone/register.js&quot;&gt;&lt;/script&gt;</p>
<p>Or, if you're using a module loader / bundler, configure it so that can-zone/register is placed above all others in the bundle.&quot;&gt;docs</a>(https://github.com/canjs/can-zone/blob/master/docs/register.md) about ensuring can-zone is registered properly.*</p>
...
<p>The latter form is useful so that you have a closure specific to that <a href="can-zone.html" title="A library that tracks asynchronous activity and lets you know when it has completed. Useful when you need to call a function and wait for all async behavior to complete, such as when performing server-side rendering.">Zone</a>.</p>
    ...
    <section class="body">
    <h2>Tasks</h2>
<p>JavaScript uses various task queues (and a microtask queue) to run JavaScript in the event loop. See <a href="https://jakearchibald.com/2015/tasks-microtasks-queues-and-schedules/">this article</a> and <a href="http://stackoverflow.com/questions/25915634/difference-between-microtask-and-macrotask-within-an-event-loop-context">this StackOverflow answer</a> to learn more.</p>
<p>For can-zone to work we have to override various task-creating functionality, this is the list of what we currently implement:</p>
...
<p>This gives you a <a href="can-zone.html" title="A library that tracks asynchronous activity and lets you know when it has completed. Useful when you need to call a function and wait for all async behavior to complete, such as when performing server-side rendering.">Zone</a> from which you can run code using <a href="can-zone.prototype.run.html" title="">zone.run</a>:</p>
<pre><code class="language-js">var Zone = require(&quot;can-zone&quot;);
