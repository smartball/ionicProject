...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-observe/docs/can-observe.md
	@module can-observe
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - can-observe</title>
...
<div class="breadcrumb">
	...
	<li><a href="can-observe.html">can-observe</a></li>
	...
	<div class="page-type">
		<h1>can-observe</h1>
			<ul class="title-social">
				...
				<li>
					<a class="npm-button" href="https://www.npmjs.com/package/can-observe">
						<img src="https://img.shields.io/badge/npm%20package-1.0.0-brightgreen.svg" alt="npm package badge" />
					</a>
				</li>
				<li>
					<a class="github-button nav-social" href="https://github.com/canjs/can-observe" data-show-count="true">Star</a>
				</li>
				
				
				<li>
					<a class="button-link" href="//github.com/canjs/can-observe/edit/master/docs/can-observe.md">Edit on GitHub</a>
				</li>
...
<div class="signature">
  ...
  <p>Create an observable object that acts as a <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy">proxy</a> for a target object.</p>
...
dog.name = 'Wilbur'; // -&gt; &quot;&lt;p&gt;dog's name is Wilbur&lt;/p&gt;&quot; on the document body
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>target</b> <code>{Object}</code>: <p>The object from which an observable instance is created.</p>
</li>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{Proxy}</code>: <p>A proxy for the target object.</p>
</p>
  </div>
    ...
    <section class="body">
    <h2>Use</h2>
<p>Using <code>can-observe</code> allows you to create observable objects where any property added is immediately observable, including nested objects. This makes <code>can-observe</code> ideal for use-cases where the data may be dynamic, or where the more rigid approach of <a href="can-define.html" title="Exports the define method that defines observable properties and their behavior on a prototype object.">can-define</a> is unneeded.</p>
<p>To use <code>can-observe</code> call the <code>observe()</code> method with an object. This will return a <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy">Proxy</a> object where any changes will reflect back on the target object. Nested objects will be observed lazily when they are accessed or set dynamically after initialization on a <code>can-observe</code> proxy object.</p>
<p>To listen for changes on a property, use <a href="can-reflect/observe.onKeyValue.html" title="Register an event handler on a MapLike object, based on a key change">canReflect.onKeyValue</a>. Pass the handler as the third argument which will be triggered when a new value is set on the proxy. In the example below, updating <code>dog.name</code> to <code>&quot;Wilbur&quot;</code> will trigger the callback that will <code>console.log</code> the new value <code>&quot;Wilbur&quot;</code>.</p>
<pre><code class="language-js">var canReflect = require(&quot;can-reflect&quot;);
var dog = observe({});
...
<p><code>can-observe</code> can be combined with any other CanJS observable type, like <a href="can-define.html" title="Exports the define method that defines observable properties and their behavior on a prototype object.">can-define</a> or <a href="can-compute.html" title="Create an observable value.">can-compute</a>. In this example we create a compute that changes when a can-observe proxy changes. Note that with computes we use <a href="can-reflect/observe.onValue.html" title="Register an event handler on an observable ValueLike object, based on a change in its value">canReflect.onValue</a> to set up the event listener and handler.</p>
...
<p>Any Object property in a <code>can-observe</code> will be replaced with a <code>can-observe</code> observed Proxy on read or write.  This allows deep path traversal in objects, with observable changes all along the way.</p>
<pre><code class="language-js">var observe = require(&quot;can-observe&quot;);
...
<h2>ES6 Classes</h2>
<p><code>can-observe</code> is specifically designed to work with ES6 classes.  To make view models for your <a href="can-component.html" title="Create a custom element that can be used to manage widgets or application logic.">can-components</a> from ES6 classes, only a few lines of constructor code are necessary:</p>
<pre><code class="language-js">import observe from (&quot;can-observe&quot;);
...
<pre><code class="language-html">&lt;my-widget messageFromParent:from=&quot;'world'&quot; /&gt;

&lt;!-- above tag will contain &quot;&lt;p&gt;Hello, world!&lt;/p&gt;&quot; on render --&gt; 
</code></pre>
<h2>Browser support</h2>
<p>can-observe uses the Proxy feature of JavaScript to observe arbitrary properties. Proxies are available in <a href="http://caniuse.com/#feat=proxy">all modern browsers</a>.</p>
<p>A <a href="https://github.com/GoogleChrome/proxy-polyfill">polyfill is available</a> that brings Proxies back to IE9, with the caveat that only existing properties on the target object can be observed. This means this code:</p>
<pre><code class="language-js">var person = observe({first: '', last: ''});
</code></pre>
<p>The <em>first</em> and <em>last</em> properties are observable in older browsers, but any other property added would not be. To ensure maximum compatibility make sure to give all properties a default value.</p>
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-observe/docs/can-observe.md"},"description":"Create an observable object. \n","type":"module","title":"","types":[{"type":"function","returns":{"types":[{"type":"undefined"}]},"params":[]}],"name":"can-observe","parent":"can-ecosystem","package":{"author":{"name":"Bitovi"},"bugs":{"url":"https://github.com/canjs/can-observe/issues"},"bundleDependencies":false,"dependencies":{"can-cid":"^1.0.1","can-event":"^3.7.5","can-namespace":"^1.0.0","can-observation":"^3.0.7","can-reflect":"^1.4.2","can-symbol":"^1.2.0"},"deprecated":false,"description":"Like can.Map, but without the .attr method.","devDependencies":{"bit-docs":"0.0.7","can-compute":"^3.0.5","can-stache":"^3.0.19","can-util":"^3.2.2","detect-cyclic-packages":"^1.1.0","jshint":"^2.9.1","steal":"^1.0.0","steal-qunit":"^1.0.0","steal-tools":"^1.9.1","testee":"^0.3.1"},"homepage":"https://github.com/canjs/can-observe","keywords":["CanJS"],"license":"MIT","main":"can-observe.js","name":"can-observe","repository":{"type":"git","url":"git+https://github.com/canjs/can-observe.git"},"scripts":{"build":"node build.js","detect-cycle":"detect-cyclic-packages --ignore done-serve","jshint":"jshint *.js --config","postversion":"git push --tags && git checkout master && git branch -D release && git push","preversion":"npm test && npm run build","release:major":"npm version major && npm publish","release:minor":"npm version minor && npm publish","release:patch":"npm version patch && npm publish","release:pre":"npm version prerelease && npm publish","test":"npm run detect-cycle && npm run jshint && npm run testee","testee":"testee test.html --browsers firefox","version":"git commit -am \"Update dist for release\" && git checkout -b release && git add -f dist/"},"version":"1.0.0"},"signatures":[{"code":"observe(target)","description":"\n\nCreate an observable object that acts as a [proxy](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy) for a target object.\n\n```js\nvar stache = require(\"can-stache\");\nvar dog = observe({});\n\nvar frag = stache(\"<p>dog's name is {{name}}</p>\")(dog);\ndocument.body.appendChild(frag);\n\ndog.name = 'Wilbur'; // -> \"<p>dog's name is Wilbur</p>\" on the document body\n```\n","params":[{"types":[{"type":"Object","options":[]}],"name":"target","description":"The object from which an observable instance is created.\n"}],"returns":{"types":[{"type":"Proxy"}],"description":"A proxy for the target object.\n"}}],"_curParam":{"types":[{"type":"Object","options":[]}],"name":"target","description":"The object from which an observable instance is created.\n"},"_curReturn":{"types":[{"type":"Proxy"}],"description":"A proxy for the target object.\n"},"comment":" ","pathToRoot":".."};
</script>
</article>
