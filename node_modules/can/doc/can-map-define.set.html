...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-map-define/docs/set.md
	@function can-map-define.set
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
...
<div class="breadcrumb">
	...
	<li><a href="can-map-define.set.html">set</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>set</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-map-define/edit/master/docs/set.md">Edit on GitHub</a>
				</li>
    ...
    <p>Specify what happens when a value is set on a map attribute.</p>
...
<div class="signature">
  ...
  <p>A set function defines the behavior of what happens when a value is set on a
<a href="can-map.html" title="Create observable objects.">Map</a>. It is typically used to:</p>
<ul>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>newVal</b> <code>{*}</code>: <p>The <a href="can-map-define._type.html" title="Converts a value passed to attr into an appropriate value.">type function</a> coerced value the user intends to set on the
<a href="can-map.html" title="Create observable objects.">Map</a>.</p>
</li>
        
          <li><b>setValue</b> <code>{function(newValue)}</code>: <p>A callback that can set the value of the property
asyncronously.</p>
</li>
...
to whatever was passed to <a href="can-map.prototype.attr.html" title="Get or set properties on a Map.">attr</a>.</li>
...
updated until <code>setValue</code> is called.</li>
    ...
    <section class="body">
    <h2>Use</h2>
<p>An attribute's <code>set</code> function can be used to customize the behavior of when an attribute value is set
via <a href="can-map.prototype.attr.html" title="Get or set properties on a Map."><code>.attr()</code></a>.  Lets see some common cases:</p>
<h4>Side effects</h4>
...
<p>The following makes changing <code>makeId</code> remove the <code>modelId</code> property:</p>
<pre><code>define: {
  makeId: {
...
<p>With 1 argument, <code>undefined</code> will remove the property.</p>
<pre><code>MyMap = Map.extend({
  define: {
...
that <code>setValue</code> will be called:</p>
<pre><code>MyMap = Map.extend({
  define: {
...
<p>For example, in the example below, Paginator can.Map includes a <code>page</code> property, which derives its value entirely from other properties (limit and offset).  If something tries to set the <code>page</code> directly, the set method will set the value of <code>offset</code>:</p>
...
<h2>Merging</h2>
<p>By default, if a value returned from a setter is an object, array, can.Map, or can.List, the effect will be to replace the property with the new object completely.</p>
<pre><code>Contact = Map.extend({
  define: {
    info: {
      set: function(newVal){
        return newVal;
...
<p>By contrast, if you access a property of a Map using <code>.attr</code>, then change it by calling <code>.attr</code> on it directly, the new properties will be merged with the existing nested Map, not replaced.</p>
<pre><code>var contact = new Map({
  'info' : {'breath' : 'smells like roses'}
});
var newInfo = {'teeth' : 'shiny and clean'};
contact.attr('info').attr(newInfo); // info is now a merged object
</code></pre>
<p>If you would rather have the new Map or List merged into the current value, not replaced, call
...
<p>By default, calls to set methods are wrapped in a call to <a href="can-event/batch/batch.start.html" title="Begin an event batch.">batch.start</a> and <a href="can-event/batch/batch.stop.html" title="End an event batch.">batch.stop</a>, so if a set method has side effects that set more than one property, all these sets are wrapped in a single batch for better performance.</p>
