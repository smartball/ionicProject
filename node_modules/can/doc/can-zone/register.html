...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-zone/docs/register.md
	@module can-zone/register
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - can-zone/register</title>
...
<div class="breadcrumb">
  ...
  <section class="title">
	<div class="page-type">
		<h1>can-zone/register</h1>
			<ul class="title-social">
				
				
				<li>
					<a class="button-link" href="//github.com/canjs/can-zone/edit/master/docs/register.md">Edit on GitHub</a>
				</li>
    ...
    <p>In order to do it's magic, <a href="../can-zone.html" title="A library that tracks asynchronous activity and lets you know when it has completed. Useful when you need to call a function and wait for all async behavior to complete, such as when performing server-side rendering.">can-zone</a> has to register handlers for all of the common JavaScript async operations. If you have code (or a dependency with this code) that does:</p>
<pre><code class="language-js">var st = setTimeout;
</code></pre>
<p>And this module loads before can-zone, any time <code>st</code> is used we won't be able to track that within the Zone.</p>
<p>To work around this, <strong>can-zone/register</strong> is used as a script that you run before any other modules.</p>
<h3>In Node</h3>
<pre><code class="language-js">require(&quot;can-zone/register&quot;);
</code></pre>
<p>At the top of your entry-point script.</p>
<h3>In the Browser</h3>
<p>You can either add a script tag above all others:</p>
<pre><code class="language-js">&lt;script src=&quot;node_modules/can-zone/register.js&quot;&gt;&lt;/script&gt;
</code></pre>
<p>Or, if you're using a module loader / bundler, configure it so that can-zone/register is placed above all others in the bundle.</p>
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-zone/docs/register.md"},"description":" \nIn order to do it's magic, [can-zone] has to register handlers for all of the common JavaScript async operations. If you have code (or a dependency with this code) that does:\n\n```js\nvar st = setTimeout;\n```\n\nAnd this module loads before can-zone, any time `st` is used we won't be able to track that within the Zone.\n\nTo work around this, **can-zone/register** is used as a script that you run before any other modules.\n\n### In Node\n\n```js\nrequire(\"can-zone/register\");\n```\n\nAt the top of your entry-point script.\n\n### In the Browser\n\nYou can either add a script tag above all others:\n\n```js\n<script src=\"node_modules/can-zone/register.js\"><\/script>\n```\n\nOr, if you're using a module loader / bundler, configure it so that can-zone/register is placed above all others in the bundle.\n\n","type":"module","title":"can-zone/register","types":[{"type":"function","returns":{"types":[{"type":"undefined"}]},"params":[]}],"name":"can-zone/register","parent":"can-zone.modules","pathToRoot":"../.."};
</script>
</article>
