...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-zone/docs/zones/timeout.md
	@module can-zone/timeout
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - can-zone/timeout</title>
...
Using these hooks you can do things like create timers and override global variables that will change the shape of code that runs within the Zone.">
	...
	<ul>
					...
					<li class="
           						...
           						">
						<a class="typedef"
							href="timeout.TimeoutError.html"
							title="A special type of Error that also includes the number of milliseconds that were waited before timing out.
...
<div class="breadcrumb">
  ...
  <section class="title">
	<div class="page-type">
		<h1>can-zone/timeout</h1>
			<ul class="title-social">
				
				
				<li>
					<a class="button-link" href="//github.com/canjs/can-zone/edit/master/docs/zones/timeout.md">Edit on GitHub</a>
				</li>
...
<div class="signature">
  ...
  <p>Creates a <a href="../can-zone.ZoneSpec.html" title="A ZoneSpec is the way you tap into the lifecycle hooks of a Zone. The hooks are described below.
Using these hooks you can do things like create timers and override global variables that will change the shape of code that runs within the Zone.">ZoneSpec</a> that you can use as a plugin for your <a href="../can-zone.html" title="A library that tracks asynchronous activity and lets you know when it has completed. Useful when you need to call a function and wait for all async behavior to complete, such as when performing server-side rendering.">Zone</a> in order to timeout after a certain length of time (as <code>ms</code>).</p>
<p>If the Zone times out it's <a href="../can-zone.prototype.run.html" title="">run promise</a> will be rejected with a &lt;a href=&quot;timeout.TimeoutError.html&quot; title=&quot;A special type of Error that also includes the number of milliseconds that were waited before timing out.
The error object is included with the timeout module:
var timeout = require(&quot;can-zone/timeout&quot;);</p>
<p>var TimeoutError = timeout.TimeoutError;
// Maybe use this to check <code>instanceof</code>.&quot;&gt;TimeoutError</a>, a special error that also includes the number of milliseconds waited before timing out.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>ms</b> <code>{Number}</code>: <p>The number of milliseconds to wait before timing out the <a href="../can-zone.html" title="A library that tracks asynchronous activity and lets you know when it has completed. Useful when you need to call a function and wait for all async behavior to complete, such as when performing server-side rendering.">Zone</a>.</p>
</li>
...
Using these hooks you can do things like create timers and override global variables that will change the shape of code that runs within the Zone.">ZoneSpec</a>}</code>: <p>A ZoneSpec that can be passed as a plugin.</p>
</p>
  </div>
    ...
    <section class="body">
    <h2>Use</h2>
<p>The timeout zone allows you to specify a timeout for your Zone. If the Zone promise doesn't resolve before timing out, the Zone promise will be rejected by the plugin.</p>
<p>The <strong>timeout</strong> zone is a function that takes a timeout in milliseconds.</p>
<p>The Promise will reject with a special type of Error, a <a href="timeout.TimeoutError.html" title="A special type of Error that also includes the number of milliseconds that were waited before timing out.
The error object is included with the timeout module:
var timeout = require(&quot;can-zone/timeout&quot;);

var TimeoutError = timeout.TimeoutError;
// Maybe use this to check `instanceof`.">TimeoutError</a>.</p>
<pre><code class="language-js">var Zone = require(&quot;can-zone&quot;);
var timeout = require(&quot;can-zone/timeout&quot;);
var TimeoutError = timeout.TimeoutError;
