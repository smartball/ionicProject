...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-map/docs/prototype.attr.md
	...
	<div class="page-type">
		<h1>attr</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-map/edit/master/docs/prototype.attr.md">Edit on GitHub</a>
				</li>
...
<p>As shown above, <code>attr</code> enables reading and setting deep properties so special care must be taken when property names include dots (<code>.</code>).  When setting a property containing dots, <code>attr</code> looks for an existing container object in the path.  If found, it will repeat the process for the child Map and the rest of the path; if not, any remaining path (dots included) becomes the property key to be set on the container.</p>
...
<p>A property <code>foo</code> and a property <code>foo.bar</code> will be in conflict with each other; when reading <code>'foo.bar'</code> with <code>attr</code>, the full string <code>foo.bar</code> takes precedence, but when writing, <code>foo</code> takes precedence and <code>foo.bar</code> cannot be written to.  For this reason, it is inadvisable to set properties that create these conflicts.</p>
<pre><code>var person = new Map({
    'first.name': 'Alice',
    'first': {
        'name': 'Amy'
