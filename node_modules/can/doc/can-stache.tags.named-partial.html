...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-stache/docs/tags/named-partial.md
	@function can-stache.tags.named-partial
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - {{&lt;partialName}}</title>
...
<div class="breadcrumb">
	...
	<li><a href="can-stache.tags.named-partial.html">{{&lt;partialName}}</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>{{&lt;partialName}}</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-stache/edit/master/docs/tags/named-partial.md">Edit on GitHub</a>
				</li>
    ...
    <p>Create an inline named partial within the current template.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>{{&lt;partialName}}BLOCK{{/partialName}}</code>
  </h2>
  <p>Creates a reusable sub-template from <code>BLOCK</code> named <code>partialName</code> that can be rendered recursively or in the current scope using <a href="can-stache.tags.partial.html" title="Render another template within the current template.">{{&gt;partialName}}</a>.</p>
<pre><code class="language-handlebars">{{&lt;addressTemplate}}
    &lt;div&gt;{{street}}, {{city}}&lt;/div&gt;
{{/addressTemplate}}
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>partialName</b> <code>{String}</code>: <p>The name of the partial.</p>
</li>
        
          <li><b>BLOCK</b> <code>{<a href="can-stache.sectionRenderer.html" title="Renders a section. These functions are usually provided as .fn and .inverse on a stache helper's options.">sectionRenderer</a>(context, helpers)}</code>: <p>a template to be captured and rendered later.</p>
</li>
    ...
    <section class="body">
    <h2>Use</h2>
<p>Named partials are sub-templates in a larger template that aren’t rendered until referenced by the <a href="can-stache.tags.partial.html" title="Render another template within the current template.">partial tag</a>. They can be referenced any number of times with different contexts.</p>
<p>Given this data:</p>
...
<p>Named partials can also have a template block that references its own name in a <a href="can-stache.tags.partial.html" title="Render another template within the current template.">partial tag</a>, which creates recursion. (So make sure you avoid infinite loops!)</p>
<p>Given this data:</p>
<pre><code class="language-js">{
    yayRecursion: {
        name: &quot;Root&quot;,
...
<p>This template:</p>
<pre><code class="language-handlebars">{{&lt;recursive}}
...
<p>Would result in:</p>
<pre><code class="language-html">&lt;div&gt;Root &lt;b&gt;Type:&lt;/b&gt; Branch&lt;/div&gt;
...
<h2>Too Much Recursion</h2>
<p>When working with recursive named partials, be aware that by default, expressions will walk up the context chain if the property is not found in the current context.</p>
<p>So if your data and template looks like this:</p>
<p>Given this data:</p>
<pre><code class="language-js">{
    yayRecursion: {
        name: &quot;Root&quot;,
...
<p>Will recurse on <code>nodes</code> and your output will be something like this:</p>
<pre><code class="language-html">&lt;div&gt;Root &lt;b&gt;Type:&lt;/b&gt; Branch&lt;/div&gt;
...
<p>This is because when it’s rendering that named partial with “Problem Child” as the context, the template sees <code>nodes</code> here: <code>{{#each(nodes)}}</code>, then it checks the current context (Problem Child), doesn't find anything called <code>nodes</code>, then moves up the scope to its parent context to check for <code>nodes</code>. Since <code>nodes</code> is on the parent (and contains the Problem Child), it uses that for the <code>#each()</code> and you’re stuck in infinite recursion.</p>
<p>To avoid that, it’s best practice to always be specific about the context for your expressions within a named partial:</p>
<pre><code class="language-handlebars">{{&lt;recursive}}
    &lt;div&gt;{{./name}} &lt;b&gt;Type:&lt;/b&gt; {{#if(./nodes.length)}}Branch{{else}}Leaf{{/if}}&lt;/div&gt;
    {{#each(./nodes)}}
        {{&gt;recursive .}}
...
<p>which prevents the default behavior for expressions to look up the context chain.</p>
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-stache/docs/tags/named-partial.md"},"description":"\nCreate an inline named partial within the current template.\n","title":"{{<partialName}}","name":"can-stache.tags.named-partial","type":"function","parent":"can-stache.tags","order":6,"signatures":[{"code":"{{<partialName}}BLOCK{{/partialName}}","description":"\n\nCreates a reusable sub-template from `BLOCK` named `partialName` that can be rendered recursively or in the current scope using [can-stache.tags.partial {{>partialName}}].\n\n```handlebars\n{{<addressTemplate}}\n\t<div>{{street}}, {{city}}</div>\n{{/addressTemplate}}\n\n<div>\n\t{{#with(business.address)}}\n\t\t{{>addressTemplate}}\n\t{{/with}}\n</div>\n<ul>\n\t{{#each(business.people)}}\n\t\t<li>\n\t\t\t{{fullName}}, {{birthday}}\n\t\t\t{{>addressTemplate address}}\n\t\t</li>\n\t{{/each}}\n</ul>\n```\n","params":[{"types":[{"type":"String"}],"name":"partialName","description":"The name of the partial.   \n"},{"types":[{"type":"can-stache.sectionRenderer"}],"name":"BLOCK","description":"a template to be captured and rendered later.\n\n\n"}]}],"_curParam":{"types":[{"type":"can-stache.sectionRenderer"}],"name":"BLOCK","description":"a template to be captured and rendered later.\n\n\n"},"comment":" ","pathToRoot":".."};
</script>
</article>
