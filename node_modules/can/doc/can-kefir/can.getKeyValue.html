...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-kefir/doc/can.getKeyValue.md
	@property can-kefir/can.getKeyValue
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
...
<div class="breadcrumb">
	...
	<li><a href="can.getKeyValue.html">can.getKeyValue</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>can.getKeyValue</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-kefir/edit/master/doc/can.getKeyValue.md">Edit on GitHub</a>
				</li>
    ...
    <p>Used by <a href="../can-reflect.getKeyValue.html" title="Get the value of a named property on a MapLike object.">can-reflect.getKeyValue</a> to read the stream's last emitted <code>value</code> or <code>error</code> and make those properties observable
by <a href="../can-observation.html" title="Provides a mechanism to notify when an observable has been read and a
way to observe those reads called within a given function.">can-observation</a>.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>stream[@@can.getKeyValue](key)</code>
  </h2>
  <p><a href="../can-reflect.getKeyValue.html" title="Get the value of a named property on a MapLike object.">can-reflect.getKeyValue</a> will use this function
to read the last emitted <code>value</code> or <code>error</code> by this stream.</p>
<p>Note, <code>streams</code> only have a value if <a href="../can-reflect/observe.onKeyValue.html" title="Register an event handler on a MapLike object, based on a key change">onKeyValue</a> is used to listen to changes.  Use
Kefir <code>properties</code> if you want to always be able to read a value.</p>
<p>The following creates a stream that immediately emits a value.  When <code>onKeyValue</code>
is used to listen to changes, it will immediately log 1.  However,
that value is preserved so <code>getKeyValue</code> can still be used:</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>name</b> <code>{String}</code>: <p>Either <code>value</code> for values emitted by the stream or <code>error</code> for
error values emitted by the stream.</p>
</li>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{*}</code>: <p>The value at that key.  The value will be stable for streams that
have already been bound by <code>onKeyValue</code> or Kefir properties.</p>
</p>
  </div>
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-kefir/doc/can.getKeyValue.md"},"description":"Used by [can-reflect.getKeyValue can-reflect.getKeyValue] to read the stream's last emitted `value` or `error` and make those properties observable\nby [can-observation].\n\n","types":[{"type":"function","returns":{"types":[{"type":"undefined"}]},"params":[]}],"title":"can.getKeyValue","name":"can-kefir/can.getKeyValue","type":"property","parent":"can-kefir/symbols","signatures":[{"code":"stream[@@can.getKeyValue](key)","description":"\n\n[can-reflect.getKeyValue can-reflect.getKeyValue] will use this function\nto read the last emitted `value` or `error` by this stream.  \n\nNote, `streams` only have a value if [can-reflect/observe.onKeyValue] is used to listen to changes.  Use\nKefir `properties` if you want to always be able to read a value.\n\n\nThe following creates a stream that immediately emits a value.  When `onKeyValue`\nis used to listen to changes, it will immediately log 1.  However,\nthat value is preserved so `getKeyValue` can still be used:\n\n\n```js\nvar Kefir = require(\"can-kefir\");\n\nvar stream = Kefir.stream(function(emit){\n\temit.value(1);\n});\n\ncanReflect.onKeyValue(stream, \"value\", function(newVal){\n\tconsole.log(\"new value\", newVal);\n}); // logs 1\n\ncanReflect.getKeyValue(stream,\"value\") //-> 1\n```\n","params":[{"types":[{"type":"String"}],"name":"name","description":"Either `value` for values emitted by the stream or `error` for\nerror values emitted by the stream.\n"}],"returns":{"types":[{"type":"*"}],"description":"The value at that key.  The value will be stable for streams that\nhave already been bound by `onKeyValue` or Kefir properties.\n"}}],"_curParam":{"types":[{"type":"String"}],"name":"name","description":"Either `value` for values emitted by the stream or `error` for\nerror values emitted by the stream.\n"},"_curReturn":{"types":[{"type":"*"}],"description":"The value at that key.  The value will be stable for streams that\nhave already been bound by `onKeyValue` or Kefir properties.\n"},"pathToRoot":"../.."};
</script>
</article>
