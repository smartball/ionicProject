...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-connect-ndjson/docs/streamError.md
	@property can-connect-ndjson/DefineList.prototype.streamError
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - streamError</title>
...
<div class="breadcrumb">
	...
	<li><a href="DefineList.prototype.streamError.html">streamError</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>streamError</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-connect-ndjson/edit/master/docs/streamError.md">Edit on GitHub</a>
				</li>
    ...
    <p>Set on the list if the underlying stream has an error.</p>
...
<div class="signature">
  ...
  <p>Below is an example using <code>streamError</code> with a <code>DefineList</code> named <code>todos</code>:</p>
<pre><code>const todos = Todo.getList({}).then(todos =&gt; {
  return todos;
}).then(todos =&gt; {
...
<p>Here we are using <code>streamError</code> in a <code>can-stache</code> template to communicate to your users that data is still being recieved from the stream:</p>
<pre><code>{{#if todos.value.streamError}}
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-connect-ndjson/docs/streamError.md"},"description":"Set on the list if the underlying stream has an error. \n\n","types":[{"type":"Object","options":[],"description":"\n\nBelow is an example using `streamError` with a `DefineList` named `todos`:\n\n```\nconst todos = Todo.getList({}).then(todos => {\n  return todos;\n}).then(todos => {\n  if (todos.value.streamError) {\n    console.error(todos.value.streamError)\n  }\n  return todos;\n});\n\n```\n\nHere we are using `streamError` in a `can-stache` template to communicate to your users that data is still being recieved from the stream:\n\n```\n{{#if todos.value.streamError}}\n  <li style=\"color:red\">NDJSON Stream Error: {{todos.value.streamError.message}}</li>\n{{/if}}\n```"}],"title":"streamError","name":"can-connect-ndjson/DefineList.prototype.streamError","type":"property","parent":"can-connect-ndjson/DefineList","pathToRoot":"../.."};
</script>
</article>
