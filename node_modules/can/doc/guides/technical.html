...
is read for the first time.">value</a>, and <a href="../can-define.types.type.html" title="Converts a value set on an instance into an appropriate value.">type</a> conversion, <a href="../can-define.types.serialize.html" title="Defines custom serialization behavior for a property.">serialize</a>
...
<li><a href="../can-route.html" title="Manage browser history and client state by synchronizing the window.location.hash with a map.">can-route</a> two-way binds a <a href="../can-define.html" title="Exports the define method that defines observable properties and their behavior on a prototype object.">can-define</a> observable
with the browser’s URL.</li>
<li><a href="../can-connect.html" title="can-connect provides persisted data middleware. Assemble powerful model layers for any JavaScript  project from fully modularized behaviors (i.e plugins).">can-connect</a> adds methods to a <a href="../can-define.html" title="Exports the define method that defines observable properties and their behavior on a prototype object.">can-define</a> observable,
...
<ul>
<li><a href="../can-connect/can/map/map.getList.html" title="Method added to the configured map._Map type. Retrieves a map._List of
map._Map instances via the connection.">Get a list</a> of Todos
<pre><code class="language-js">Todo.getList({complete: true}).then(function(todos){})
</code></pre></li>
<li><a href="../can-connect/can/map/map.get.html" title="Method added to the configured map._Map type. Retrieves an instance of the
map._Map type via the connection.">Get</a> a single Todo
<pre><code class="language-js">Todo.get({_id: 6}).then(function(todo){})
</code></pre></li>
<li><a href="../can-connect/can/map/map.prototype.save.html" title="Save the instance's data to the service via the connection.">Create</a> a Todo
<pre><code class="language-js">var todo = new Todo({name: &quot;do dishes&quot;, complete: false})
todo.save().then(function(todo){})
</code></pre></li>
<li><a href="../can-connect/can/map/map.prototype.save.html" title="Save the instance's data to the service via the connection.">Update</a> an <a href="../can-connect/can/map/map.prototype.isNew.html" title="Returns if the instance has not been loaded from or saved to the data source.">already created</a> Todo
...
<p>Check out these tests running in <a href="https://jsbin.com/lulucuboni/1/edit?html,js,output">this JS&nbsp;Bin</a>.</p>
<h3>Flexible</h3>
...
<a href="../can-stache-bindings.toChild.html" title="One-way bind a value in the parent scope to the ViewModel or element.">toChild:from</a> bindings like:
...
<pre><code class="language-html">{{#players}}
    &lt;h2&gt;{{name}}&lt;/h2&gt;
...
<pre><code class="language-js">task.on(&quot;dueDate&quot;, function(ev, dueDate){
...
<li><code>prop:from=&quot;key&quot;</code> for <a href="../can-stache-bindings.toChild.html" title="One-way bind a value in the parent scope to the ViewModel or element.">one-way binding to a child</a>.</li>
<li><code>prop:to=&quot;key&quot;</code> for <a href="../can-stache-bindings.toParent.html" title="One-way bind a value from the viewModel or element to the parent scope.">one-way binding to a parent</a>.</li>
<li><code>prop:bind=&quot;key&quot;</code> for <a href="../can-stache-bindings.twoWay.html" title="Two-way bind a value in the viewModel or the element to the parent scope.">two-way binding</a>.</li>
</ul>
...
<p>In addition to CRUD and real-time capabilities, <a href="../can-set.html" title="can-set is a utility for comparing sets that are represented by the parameters commonly passed to service requests.">can-set</a> and <a href="../can-connect.html" title="can-connect provides persisted data middleware. Assemble powerful model layers for any JavaScript  project from fully modularized behaviors (i.e plugins).">can-connect</a> provide lots of great features out-of-the-box:</p>
<ul>
...
<p>Using <a href="../can-connect.html" title="can-connect provides persisted data middleware. Assemble powerful model layers for any JavaScript  project from fully modularized behaviors (i.e plugins).">can-connect</a>, we’ll create a connection between a RESTful <code>/api/todos</code> service and our <code>Todo</code> instances and <code>TodoList</code> lists:</p>
...
<pre><code class="language-js">Todo.getList({completed: false}).then(function(incompleteTodos) {});
</code></pre>
...
<pre><code class="language-js">Todo.getList({priority: &quot;high&quot;}).then(function(urgentTodos) {});
</code></pre>
...
<p><a href="../can-connect.html" title="can-connect provides persisted data middleware. Assemble powerful model layers for any JavaScript  project from fully modularized behaviors (i.e plugins).">can-connect</a>’s <a href="../can-connect/constructor/store/store.instanceStore.html" title="A mapping of instances keyed by their base.id.">instance store</a> keeps a reference to every model object by <code>id</code> (but you can <a href="../can-set.props.id.html" title="Defines the identify property.">change</a> which property is used). It does two things:</p>
<ol>
...
<ul>
<li><p><strong>Explicitly:</strong> if you use <a href="../can-connect/constructor/store/store.addInstanceReference.html" title="Add a reference to the store.instanceStore so an instance can be easily looked up.">addInstanceReference</a> or call <code>.on()</code> on an instance (e.g. <code>todo.on('name', function(){})</code>)</p></li>
...
<p>When the user clicks the checkbox for the “Finish docs” todo, its <code>completed</code> property is set to <code>true</code> and it disappears from the <code>incompleteTodos</code> list when <a href="../can-connect/can/map/map.prototype.save.html" title="Save the instance's data to the service via the connection.">.save()</a> is called.</p>
<p>This is made possible by two things:</p>
<ul>
<li><p>The <a href="../can-connect/constructor/store/store.listStore.html" title="A mapping of lists keyed by their base.listSet.">list store</a> contains all of the lists loaded from the server. It’s memory safe so it won’t leak.</p></li>
...
<pre><code class="language-js">var socket = io('https://example.com');
...
<p>Here’s an example of <a href="../can-connect/base/base.algebra.html" title="can-set set algebra used for list comparison, instance identification and membership
calculations. A way for the can-connect behaviors to understand what the properties of a request mean and act
on them.">setting up the algebra</a> for the <code>Todo.connection</code>:</p>
<pre><code class="language-js">var connect = require(&quot;can-connect&quot;);
...
<p>With CanJS, you don’t have to choose between maintainability and performance. You can decide to have each component fetch its data independently and <a href="../can-connect.html" title="can-connect provides persisted data middleware. Assemble powerful model layers for any JavaScript  project from fully modularized behaviors (i.e plugins).">can-connect</a> will intelligently combine the two requests into one.</p>
<p>This is made possible by the <a href="../can-set.html" title="can-set is a utility for comparing sets that are represented by the parameters commonly passed to service requests.">can-set</a> algebra we discussed earlier. <a href="../can-connect.html" title="can-connect provides persisted data middleware. Assemble powerful model layers for any JavaScript  project from fully modularized behaviors (i.e plugins).">can-connect</a> sees the outgoing requests, can determine that requests for <code>Todo.getList({completed: true, sort: 'completedDate'})</code> and <code>Todo.getList({completed: false, sort: 'priority'})</code> are equivalent to just one <code>Todo.getList({})</code> request, then make that single request and return the correct sorted data to each call.</p>
...
<pre><code class="language-js">Todo.get({id: 2}).then(function(todo){
  todo.projectRef.id //-&gt; 2
});
</code></pre>
<p>With populating the project data:</p>
<pre><code class="language-js">Todo.get({id: 2, populate: &quot;projectRef&quot;}).then(function(todo){
  todo.projectRef.id //-&gt; 2
});
</code></pre>
...
<pre><code class="language-html">{{#each(todos, todo=value)}}
  Name: {{todo.name}}
