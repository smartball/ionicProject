...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: docs/can-guides/commitment/recipes/todomvc-with-steal/todomvc-with-steal.md
...
<li>In addition, this application should load the <a href="https://www.npmjs.com/package/can-todomvc-test">can-todomvc-test</a> module version 1.0 and
pass it the application’s <code>ViewModel</code> instance. You will need to declare the version explicitly as different versions of this guide depend on different versions of this package.</li>
</ul>
...
<pre><code class="language-cmd">npm install can-todomvc-test@1.0 --save-dev
</code></pre>
...
is read for the first time.">value</a> behavior defines a property’s initial value like:</p>
<pre><code class="language-js">DefineMap.extend({
...
<li><p>The <a href="../../can-define/list/list.prototype.wildcardItems.html" title="Define default behavior for items in the list.">#</a> property defines the behavior of items in a list like:</p>
...
observable value is changed.">get</a> behavior defines observable computed properties like:</p>
<pre><code class="language-js">DefineMap.extend({
    propertyName: {
        get: function(){
...
<li><p>Use <a href="../../can-stache-bindings.twoWay.html" title="Two-way bind a value in the viewModel or the element to the parent scope.">value:bind</a> to setup a two-way binding in <code>can-stache</code>.  For example, the following keeps <code>name</code> and the input’s <code>value</code> in sync:</p>
<pre><code class="language-html">&lt;input  value:bind=&quot;name&quot;/&gt;
</code></pre></li>
</ul>
<h3>The solution</h3>
<p>Update <em>index.stache</em> to the following:</p>
...
<li><p><a href="../../can-connect/can/map/map.getList.html" title="Method added to the configured map._Map type. Retrieves a map._List of
map._Map instances via the connection.">Type.getList</a> gets data using the
...
<p><span line-highlight='5,10-12,only'></span></p>
...
<h3>What you need to know</h3>
<ul>
<li><p>Call <a href="../../can-connect/can/map/map.prototype.save.html" title="Save the instance's data to the service via the connection.">save</a> to update a &quot;connected&quot;
<code>Map</code> instance:</p>
<pre><code>map.save();
</code></pre>
<p><code>save()</code> can also be called by an <a href="../../can-stache-bindings.event.html" title="Respond to events on elements or component ViewModels.">on:event</a> binding.</p></li>
<li><p><a href="../../can-connect/can/map/map.prototype.isSaving.html" title="Returns if the instance is currently being saved.">isSaving</a> returns true when <code>.save()</code>
...
on set logic.">real-time</a> behavior works.</p></li>
<li><p>Delete a record on the server with <a href="../../can-connect/can/map/map.prototype.destroy.html" title="Delete an instance from the service via the connection.">destroy</a> like:</p>
<pre><code class="language-js">map.destroy()
</code></pre></li>
<li><p><a href="../../can-connect/can/map/map.prototype.isDestroying.html" title="Returns if the instance is currently being destroyed.">isDestroying</a> returns true when <code>.destroy()</code>
has been called, but has not resolved yet.</p>
...
<li><p>The <a href="../../can-define.types.ValueConstructor.html" title="Provides a constructor function to be used to provide a default value for a property.">Value</a> behavior creates a default value by using <code>new Value</code> to initialize the value when
...
<li><p>Use <a href="../../can-stache-bindings.toChild.html" title="One-way bind a value in the parent scope to the ViewModel or element.">toChild:from</a> to pass a value from the scope to a component:</p>
...
<p><span line-highlight='34-44,only'></span>
Update <em>index.js</em> to the following:</p>
<pre><code class="language-js">// index.js
var view = require(&quot;./index.stache&quot;);
var DefineMap = require(&quot;can-define/map/&quot;);
var Todo = require(&quot;~/models/todo&quot;);
...
<p><span line-highlight='14-19,only'></span>
Update <em>index.stache</em> to the following:</p>
<pre><code class="language-html">&lt;!-- index.stache --&gt;
&lt;can-import from=&quot;~/components/todo-create/&quot;/&gt;
&lt;can-import from=&quot;~/components/todo-list/&quot;/&gt;
...
<p><span line-highlight='45-49,only'></span>
Update <em>index.stache</em> to the following:</p>
<pre><code class="language-html">&lt;!-- index.stache --&gt;
&lt;can-import from=&quot;~/components/todo-create/&quot;/&gt;
&lt;can-import from=&quot;~/components/todo-list/&quot;/&gt;
...
<p><span line-highlight='31-32,only'></span></p>
...
<h3>What you need to know</h3>
<ul>
<li><p><a href="../../can-route.html" title="Manage browser history and client state by synchronizing the window.location.hash with a map.">can-route</a> is used to connect a <code>DefineMap</code>’s properties
to the URL.  This is done with <a href="../../can-route.data.html" title="This is the internal map underlying can-route. It can be set in order to cross-bind a top level state object (Application ViewModel) to can-route.">data</a> like:</p>
<pre><code class="language-js">route.data = new AppViewModel();
</code></pre></li>
<li><p><a href="../../can-route.html" title="Manage browser history and client state by synchronizing the window.location.hash with a map.">can-route</a> can create pretty routing rules.  For example,
...
that use <a href="../../can-route.html" title="Manage browser history and client state by synchronizing the window.location.hash with a map.">can-route</a>.</p>
...
<p><span line-highlight='5,9-26,39-41,only'></span>
Update <em>index.stache</em> to the following:</p>
<pre><code class="language-html">&lt;!-- index.stache --&gt;
&lt;can-import from=&quot;~/components/todo-create/&quot;/&gt;
&lt;can-import from=&quot;~/components/todo-list/&quot;/&gt;
...
<p><span line-highlight='4,23-26,29-32,35-38,only'></span>
<strong>Success!</strong> You’ve completed this guide. Have questions or comments? Let us know on <a href="https://gitter.im/canjs/canjs">Gitter chat</a> or our <a href="https://forums.donejs.com/c/canjs">forums</a>!</p>
