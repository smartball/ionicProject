...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-event/batch/batch.js
	@function can-event/batch/batch.afterPreviousEvents
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - afterPreviousEvents</title>
...
All methods provided by canEvent assume that they are mixed into an object -- this should be the object dispatching the events.">
	...
	<ul>
					...
					<li class="
           						...
           						">
						<a class="function"
							href="batch.debounce.html"
							title="Provides a debounced event listener">
							debounce
						</a>
...
<div class="breadcrumb">
	...
	<li><a href="batch.afterPreviousEvents.html">afterPreviousEvents</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>afterPreviousEvents</h1>
			<ul class="title-social">
				
				
				<li>
					<a class="button-link" href="//github.com/canjs/can-event/edit/master/batch/batch.js#L495">Edit on GitHub</a>
				</li>
    ...
    <p>Run code when all previuos state has settled.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>canBatch.afterPreviousEvents(handler)</code>
  </h2>
  <p>Calls <code>handler</code> when all previously [can-event/batch/batch.trigger triggered] events have
been fired.  This is useful to know when all fired events match the current state.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>handler</b> <code>{function}</code>: <p>A function to call back when all previous events have fired.</p>
</li>
    ...
    <section class="body">
    <h2>Use</h2>
<p>With batching, it's possible for a piece of code to read some observable, and listen to
changes in that observable, but have events fired that it should ignore.</p>
<p>For example, consider a list widget that creates <code>&lt;li&gt;</code>'s for each item in the list and listens to
updates in that list and adds or removes <code>&lt;li&gt;</code>s:</p>
<pre><code class="language-js">var makeLi = function(){
...
<p>The problem with this is if someone calls <code>listWidget</code> within a batch:</p>
<pre><code class="language-js">var list = new DefineList([]);
...
<p>The problem is that list will immediately create an <code>li</code> for both <code>can-event</code> and <code>can-event/batch/</code>, and then,
when <code>canBatch.stop()</code> is called, the <code>add</code> event listener will create duplicate <code>li</code>s.</p>
<p>The solution, is to use <code>afterPreviousEvents</code>:</p>
<pre><code class="language-js">var makeLi = function(){
  return document.createElement(&quot;li&quot;)
};
...
<script type="text/javascript">
  window.docObject = {"src":{"line":494,"codeLine":572,"path":"node_modules/can-event/batch/batch.js"},"type":"function","description":"Run code when all previuos state has settled. \n","title":"afterPreviousEvents","name":"can-event/batch/batch.afterPreviousEvents","parent":"can-event/batch/batch","signatures":[{"code":"canBatch.afterPreviousEvents(handler)","description":"\n\nCalls `handler` when all previously [can-event/batch/batch.trigger triggered] events have\nbeen fired.  This is useful to know when all fired events match the current state.\n","params":[{"types":[{"type":"function","returns":{"types":[{"type":"undefined"}]},"params":[]}],"name":"handler","description":"A function to call back when all previous events have fired.\n"}]}],"_curParam":{"types":[{"type":"function","returns":{"types":[{"type":"undefined"}]},"params":[]}],"name":"handler","description":"A function to call back when all previous events have fired.\n"},"comment":" ","pathToRoot":"../../.."};
</script>
</article>
