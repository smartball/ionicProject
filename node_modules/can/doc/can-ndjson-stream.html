...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-ndjson-stream/docs/can-ndjson-stream.md
	@module can-ndjson-stream
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - can-ndjson-stream</title>
...
<div class="breadcrumb">
	...
	<li><a href="can-ndjson-stream.html">can-ndjson-stream</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>can-ndjson-stream</h1>
			<ul class="title-social">
				
				<li>
					<a class="npm-button" href="https://www.npmjs.com/package/can-ndjson-stream">
						<img src="https://img.shields.io/badge/npm%20package-0.1.6-brightgreen.svg" alt="npm package badge" />
					</a>
				</li>
				<li>
					<a class="github-button nav-social" href="https://github.com/canjs/can-ndjson-stream" data-show-count="true">Star</a>
				</li>
				
				
				<li>
					<a class="button-link" href="//github.com/canjs/can-ndjson-stream/edit/master/docs/can-ndjson-stream.md">Edit on GitHub</a>
				</li>
    ...
    <p>Parses an <a href="http://www.ndjson.org">NDJSON</a> stream into a stream of JavaScript objects.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>ndjsonStream(stream)</code>
  </h2>
  <p>The <code>can-ndjson-stream</code> module converts a stream of NDJSON to a <a href="https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream">ReadableStream</a> of JavaScript objects. It is likely that you would use this module to parse an NDJSON stream <code>response</code> object received from a <a href="https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API"><code>fetch</code></a> request to a service that sends NDJSON streams.</p>
<pre><code class="language-js">const ndjsonStream = require('can-ndjson-stream');
...
fetch('/some/endpoint')  // make a fetch request to a NDJSON stream service
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>stream</b> <code>{ReadableStream&lt;Byte&gt;}</code>: <p>A readable <a href="http://www.ndjson.org/">NDJSON</a> byte stream.</p>
</li>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{ReadableStream&lt;Object&gt;}</code>: <p>The output is a <a href="https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream">ReadableStream</a> that has the following methods:</p>
<ul>
<li>getReader()</li>
<li>cancel([optional cancellation message])</li>
</ul>
</p>
  </div>
    ...
    <section class="body">
    <h2>Use</h2>
<p>This module is typically used with <a href="https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API">fetch</a> to parse an NDJSON response stream. Follow the steps below to use <code>fetch</code> with an NDJSON stream service. See the <a href="#CreatinganNDJSONstreamservicewithNodeJS_">Creating an NDJSON stream service with NodeJS</a> section below to learn how to create a service that emits an NDJSON stream.</p>
<p>Assuming your raw data looks something like this:</p>
<pre><code class="language-js">{&quot;item&quot;:&quot;first&quot;}\n
{&quot;item&quot;:&quot;second&quot;}\n
{&quot;item&quot;:&quot;third&quot;}\n
{&quot;item&quot;:&quot;fourth&quot;}\n
</code></pre>
<p>follow these steps to make a request from an NDJSON service at <code>/some/endpoint</code>:</p>
<ol>
<li>Make a <code>fetch</code> request to an NDJSON service by passing the endpoint as an argument.</li>
<li>The service responds with a stream with each value being one line of NDJSON: <code>{&quot;item&quot;:&quot;first&quot;}\n</code></li>
<li><code>Fetch</code>'s <code>then</code> method is provided a <code>Response</code> instance, which we can parse using <code>ndjsonStream()</code> into a JavaScript <code>ReadableStream</code>.</li>
<li>Each JavaScript object in the stream can be read by calling <code>[streamName].getReader.read()</code>, which returns a promise.</li>
<li>The result of that promise will be one JS object from your NDJSON: <code>{item: &quot;first&quot;}</code></li>
</ol>
<pre><code class="language-js">  const ndjsonStream = require('can-ndjson-stream');
...
<p><a href="http://ndjson.org">NDJSON</a> is a data format that is separated into individual JSON objects with a newline character (<code>\n</code>). The 'nd' stands for newline delimited JSON. Essentially, you have some data that is formatted like this:</p>
<pre><code class="language-js">{&quot;item&quot;:&quot;first&quot;}\n
{&quot;item&quot;:&quot;second&quot;}\n
{&quot;item&quot;:&quot;third&quot;}\n
{&quot;item&quot;:&quot;fourth&quot;}\n
</code></pre>
<p>Each item above is separated with a newline and each of those can be sent individually over a stream which allows the client to receive and process the data in specified increments.</p>
<h2>Creating an NDJSON stream service with NodeJS.</h2>
<p>This is a quick start guide to getting a NDJSON stream API up and running.
It reads from a local <code>todos.ndjson</code> file and responds with a line from the
file every 500ms.</p>
<ol>
<li>Install dependencies:</li>
</ol>
<pre><code class="language-bash">$ npm i express path fs ndjson
</code></pre>
<ol start="2">
<li>Create a server.js file and copy this code:</li>
</ol>
...
<p>We use a <code>setInterval</code> to slow the stream down so that you can see the stream in action. Feel free to remove the setInterval and use a <code>while</code> loop to remove the delay.</p>
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-ndjson-stream/docs/can-ndjson-stream.md"},"description":"Parses an [NDJSON](http://www.ndjson.org) stream into a stream of JavaScript objects. \n","type":"module","title":"","types":[{"type":"function","returns":{"types":[{"type":"undefined"}]},"params":[]}],"name":"can-ndjson-stream","parent":"can-ecosystem","package":{"author":{"name":"Bitovi","email":"contact@bitovi.com","url":"http://bitovi.com"},"bugs":{"url":"https://github.com/canjs/can-ndjson-stream/issues"},"bundleDependencies":false,"dependencies":{"can-namespace":"^1.0.0"},"deprecated":false,"description":"[![Greenkeeper badge](https://badges.greenkeeper.io/canjs/can-ndjson-stream.svg)](https://greenkeeper.io/)","devDependencies":{"http-server":"^0.10.0","jshint":"^2.9.1","saucelabs":"^1.4.0","steal":"^1.0.5","steal-qunit":"^1.0.0","steal-tools":"^1.0.1","test-saucelabs":"0.0.1","testee":"^0.4.0"},"false":{},"homepage":"https://github.com/canjs/can-ndjson-stream#readme","keywords":[],"licenses":[{"type":"MIT","url":"http://opensource.org/licenses/MIT"}],"main":"can-ndjson-stream","name":"can-ndjson-stream","repository":{"type":"git","url":"git://github.com/canjs/can-ndjson-stream.git"},"scripts":{"build":"node build.js","ci":"npm run build && npm run test && node ./test-sauce-labs.js","demo":"cd demo/ && npm i && npm run start","develop":"done-serve --static --develop --port 8080","http-server":"http-server -p 3000 --silent","jshint":"jshint ./*.js --config","postpublish":"git push --tags && git checkout master && git branch -D release && git push","preversion":"npm test && npm run build","release:major":"npm version major && npm publish","release:minor":"npm version minor && npm publish","release:patch":"npm version patch && npm publish","test":"npm run jshint && node test.js","test-local":"npm run jshint && npm run testee-local","testee-local":"testee test.html --browsers chrome","version":"git commit -am \"Update version number\" && git checkout -b release && git add -f dist/"},"steal":{"main":"can-ndjson-stream","configDependencies":["live-reload"],"npmIgnore":["testee","generator-donejs","donejs-cli","steal-tools"]},"version":"0.1.6"},"signatures":[{"code":"ndjsonStream(stream)","description":"\n\nThe `can-ndjson-stream` module converts a stream of NDJSON to a [ReadableStream](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream) of JavaScript objects. It is likely that you would use this module to parse an NDJSON stream `response` object received from a [`fetch`](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) request to a service that sends NDJSON streams.\n```js\nconst ndjsonStream = require('can-ndjson-stream');\n\nfetch('/some/endpoint')  // make a fetch request to a NDJSON stream service\n  .then((response) => {\n    return ndjsonStream(response.body); //ndjsonStream parses the response.body\n\n  }).then((exampleStream) => {\n    let read;\n    exampleStream.getReader().read().then(read = (result) => {\n      if (result.done) return;\n\n      console.log(result.value);\n      exampleStream.getReader().read().then(read);\n\n    });\n  });\n```\n","params":[{"types":[{"type":"ReadableStream","template":[{"types":[{"type":"Byte"}]}]}],"name":"stream","description":"A readable [NDJSON](http://www.ndjson.org/) byte stream.  \n"}],"returns":{"types":[{"type":"ReadableStream","template":[{"types":[{"type":"Object","options":[]}]}]}],"description":"The output is a [ReadableStream](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream) that has the following methods:\n- getReader()\n- cancel([optional cancellation message])\n"}}],"_curParam":{"types":[{"type":"ReadableStream","template":[{"types":[{"type":"Byte"}]}]}],"name":"stream","description":"A readable [NDJSON](http://www.ndjson.org/) byte stream.  \n"},"_curReturn":{"types":[{"type":"ReadableStream","template":[{"types":[{"type":"Object","options":[]}]}]}],"description":"The output is a [ReadableStream](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream) that has the following methods:\n- getReader()\n- cancel([optional cancellation message])\n"},"comment":" ","pathToRoot":".."};
</script>
</article>
