...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-stache/docs/helper.md
	@typedef can-stache.helper
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - helper</title>
...
<div class="breadcrumb">
	...
	<li><a href="can-stache.helper.html">helper</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>helper</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-stache/edit/master/docs/helper.md">Edit on GitHub</a>
				</li>
    ...
    <p>A helper function passed to <a href="can-stache.registerHelper.html" title="Register a helper.">registerHelper</a>.
Given the arguments, returns the content that should be shown in the DOM
or a function that will be called on the DOM element the helper was
called on.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>function(arg..., <a href="can-stache.helperOptions.html" title="The options argument passed to a helper function when called by a helper.">options</a>)</code>
  </h2>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>arg</b> <code>{*|<a href="can-compute.html" title="Create an observable value.">can-compute</a>}</code>: <p>Arguments passed from the tag. After the helper
name, any space separated <a href="can-stache.key.html" title="A named reference to a value in the scope or helper scope in a template.">keys</a>, numbers or
strings are passed as arguments. <a href="can-stache.key.html" title="A named reference to a value in the scope or helper scope in a template.">Keys</a> that
read an observable value in <a href="can-stache/expressions/helper.html" title="">helpers</a> are passed as <a href="can-compute.computed.html" title="A derived value from other computes and observable maps.">compute</a>s .</p>
</li>
        
          <li><b>options</b> <code>{<a href="can-stache.helperOptions.html" title="The options argument passed to a helper function when called by a helper.">helperOptions</a>}</code>: <p>An options object
that is an additional argument in <code>Helper</code> expressions that is populated with optional:</p>
<ul>
...
<li>a <code>hash</code> object of the maps passed to the helper</li>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{documentFragment|String|Array|function(Node)}</code>: <p>The content to be inserted into the template or a callback function.  The content can be a:</p>
<ul>
<li><code>documentFragment</code> like those returned by <a href="can-stache.helperOptions.html" title="The options argument passed to a helper function when called by a helper.">helperOptions</a>.fn.</li>
<li><code>String</code> like <code>&quot;Hello World&quot;</code></li>
<li><code>Array</code> of strings and nodes like <code>[&quot;Hello&quot;, document.createElement('div')]</code></li>
</ul>
<p>If a <code>function(Node)</code> is returned, it will be called back with an HTML Element or text node.</p>
<p>If the helper is called on an element like:</p>
<pre><code>&lt;div {{myHelper}}&gt;
</code></pre>
<p>... the callback function will be called with the <code>div</code>.</p>
<p>If the helper is called
outside of an element's tag like:</p>
<pre><code>&lt;div&gt; {{myHelper}} &lt;/div&gt;
</code></pre>
<p>... the callback function will be called with a placeholder text node.</p>
</p>
  </div>
    ...
    <section class="body">
    <h2>Returning an element callback function</h2>
<p>If a helper returns a function, that function is called back after
the template has been rendered into DOM elements. This can
be used to create stache tags that have rich behavior.</p>
<p>If the helper is called <strong>within a tag</strong> like:</p>
<pre><code>&lt;ul {{sortable}}/&gt;
</code></pre>
<p>The returned function is called with the <code>&lt;ul&gt;</code> element:</p>
<pre><code>stache.registerHelper(&quot;sortable&quot;,function(){
  return function(el){
    $(el).slider();
  }
});
</code></pre>
<p>If the helper is called <strong>between tags</strong> like:</p>
<pre><code>&lt;ul&gt;{{items}}&lt;/ul&gt;
</code></pre>
<p>The returned function is called with a temporary text node:</p>
<pre><code>stache.registerHelper(&quot;items&quot;,function(){
  return function(textNode){
      // do something, probably replace textNode
  }
});
</code></pre>
<p>While this form of helper is still supported, it's more common
to create similar functionality with <a href="can-component.html" title="Create a custom element that can be used to manage widgets or application logic.">can-component</a> or <a href="can-view-callbacks.html" title="Registered callbacks for behaviors">can-view-callbacks</a>.</p>
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-stache/docs/helper.md"},"description":"A helper function passed to [can-stache.registerHelper]. \nGiven the arguments, returns the content that should be shown in the DOM\nor a function that will be called on the DOM element the helper was\ncalled on.\n\n","type":"typedef","title":"helper","types":[{"type":"function","returns":{"types":[{"type":"documentFragment"},{"type":"String"},{"type":"Array"},{"type":"function","returns":{"types":[{"type":"undefined"}]},"params":[{"types":[{"type":"Node"}]}]}],"description":"The content to be inserted into the template or a callback function.  The content can be a:\n\n - `documentFragment` like those returned by [can-stache.helperOptions].fn.\n - `String` like `\"Hello World\"`\n - `Array` of strings and nodes like `[\"Hello\", document.createElement('div')]`\n\nIf a `function(Node)` is returned, it will be called back with an HTML Element or text node.  \n\nIf the helper is called on an element like:\n\n```\n<div {{myHelper}}>\n```\n\n... the callback function will be called with the `div`.  \n\nIf the helper is called\noutside of an element's tag like:\n\n```\n<div> {{myHelper}} </div>\n```\n\n... the callback function will be called with a placeholder text node.  \n\n"},"params":[{"types":[{"type":"*"},{"type":"can-compute"}],"optional":true,"variable":true,"name":"arg","description":"Arguments passed from the tag. After the helper\nname, any space separated [can-stache.key keys], numbers or\nstrings are passed as arguments. [can-stache.key Keys] that\nread an observable value in [can-stache/expressions/helper helpers] are passed as [can-compute.computed]s .\n"},{"types":[{"type":"can-stache.helperOptions"}],"optional":true,"name":"options","description":"An options object\nthat is an additional argument in `Helper` expressions that is populated with optional:\n\n- `fn` and `inverse` section rendering functions\n- a `hash` object of the maps passed to the helper\n"}],"context":{"types":[{"type":"can-stache.context"}],"description":"The context the helper was\ncalled within.\n"}}],"name":"can-stache.helper","parent":"can-stache.types","_curParam":{"types":[{"type":"can-stache.helperOptions"}],"optional":true,"name":"options","description":"An options object\nthat is an additional argument in `Helper` expressions that is populated with optional:\n\n- `fn` and `inverse` section rendering functions\n- a `hash` object of the maps passed to the helper\n"},"_curReturn":{"types":[{"type":"documentFragment"},{"type":"String"},{"type":"Array"},{"type":"function","returns":{"types":[{"type":"undefined"}]},"params":[{"types":[{"type":"Node"}]}]}],"description":"The content to be inserted into the template or a callback function.  The content can be a:\n\n - `documentFragment` like those returned by [can-stache.helperOptions].fn.\n - `String` like `\"Hello World\"`\n - `Array` of strings and nodes like `[\"Hello\", document.createElement('div')]`\n\nIf a `function(Node)` is returned, it will be called back with an HTML Element or text node.  \n\nIf the helper is called on an element like:\n\n```\n<div {{myHelper}}>\n```\n\n... the callback function will be called with the `div`.  \n\nIf the helper is called\noutside of an element's tag like:\n\n```\n<div> {{myHelper}} </div>\n```\n\n... the callback function will be called with a placeholder text node.  \n\n"},"comment":" ","pathToRoot":".."};
</script>
</article>
