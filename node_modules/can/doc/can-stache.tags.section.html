...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-stache/docs/tags/section.md
	@function can-stache.tags.section
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
...
<div class="breadcrumb">
	...
	<li><a href="can-stache.tags.section.html">{{#expression}}</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>{{#expression}}</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-stache/edit/master/docs/tags/section.md">Edit on GitHub</a>
				</li>
    ...
    <p>Renders a subsection one or more times depending on the type of expression
...
<div class="signature">
  <h2 class="signature-title">
    <code>{{#KEY_EXPRESSION}}FN{{else}}INVERSE{{/KEY_EXPRESSION}}</code>
  </h2>
  <p>Renders the <code>FN</code> or <code>INVERSE</code> section one or many times depending on
the value in <code>KEY_EXPRESSION</code>.</p>
<p>If <code>KEY_EXPRESSION</code> returns an  <a href="can-util/js/is-array-like/is-array-like.html" title="can-util/js/is-array-like/is-array-like">array like object</a>,
the <code>FN</code> section will be rendered for each item in the array.  If the array like object is
empty, the <code>INVERSE</code> section will be rendered. The <a href="can-stache.helpers.each.html" title="">{{#each(expression)}}</a> helper
should generally be used for observable array-like objects as it has some performance
advantages.</p>
<pre><code>{{#items}}&lt;li&gt;{{name}}&lt;/li&gt;{{/items}}
</code></pre>
<p>If <code>KEY_EXPRESSION</code> returns a truthy value, the <code>FN</code> section will be rendered with
the truthy value.</p>
<p>If <code>KEY_EXPRESSION</code> returns a fasley value, the <code>INVERSE</code> section will be rendered with
the fasley value.</p>
<pre><code>{{#address}} {{street}} {{city}} {{/address}}
</code></pre>
<p>The closing tag can end with <code>{{/}}</code>.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>KEY_EXPRESSION</b> <code>{<a href="can-stache/expressions/key-lookup.html" title="">KeyLookup Expression</a>}</code>: <p>A key expression.
If there is no value in the scope of <code>keyOrHelper</code>, it will be treated as a <a href="can-stache/expressions/helper.html" title="">Helper Expression</a>.</p>
</li>
        
          <li><b>FN</b> <code>{<a href="can-stache.sectionRenderer.html" title="Renders a section. These functions are usually provided as .fn and .inverse on a stache helper's options.">sectionRenderer</a>(context, helpers)}</code>: <p>The truthy subsection.</p>
</li>
        
          <li><b>INVERSE</b> <code>{<a href="can-stache.sectionRenderer.html" title="Renders a section. These functions are usually provided as .fn and .inverse on a stache helper's options.">sectionRenderer</a>(context, helpers)}</code>: <p>An optional inverse section created
by using <a href="can-stache.helpers.else.html" title="">{{else}}</a>.</p>
</li>
...
<div class="signature">
  <h2 class="signature-title">
    <code>{{#CALL_EXPRESSION}}FN{{else}}INVERSE{{/CALL_EXPRESSION}}</code>
  </h2>
  <p>Works like <code>{{#KEY_EXPRESSION}}</code>, but uses the return value of
the <code>CALL_EXPRESSION</code>.</p>
<pre><code>{{#getTasksForPerson(person)}}&lt;li&gt;{{name}}&lt;/li&gt;{{/getTasksForPerson}}
</code></pre>
<p>Typically, the closing tag only include the method name and not its parameters.</p>
<p>The closing tag can end with <code>{{/}}</code>.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>CALL_EXPRESSION</b> <code>{<a href="can-stache/expressions/call.html" title="">Call Expression</a>}</code>: <p>A function that
will be called with any specified arguments.</p>
</li>
...
<div class="signature">
  <h2 class="signature-title">
    <code>{{#HELPER_EXPRESSION}}FN{{else}}INVERSE{{/HELPER_EXPRESSION}}</code>
  </h2>
  <p>Calls a <a href="can-stache.registerHelper.html" title="Register a helper.">registered helper</a> or a function in the
<a href="can-view-scope.html" title="Create a lookup node for keys.">can-view-scope</a> with an additional <a href="can-stache.helperOptions.html" title="The options argument passed to a helper function when called by a helper.">helperOptions</a> argument
that can call the <code>FN</code> or <code>INVERSE</code> helpers to build the content that
should replace these tags.</p>
<pre><code>&lt;p&gt;{{#countTo(number)}}{{num}}{{/countTo}}&lt;/p&gt;
</code></pre>
<p>Helpers, with their direct access to subsection renderers and scope
have more control over template flow.  However, they are harder to test
than methods in the view model or model.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>HELPER_EXPRESSION</b> <code>{<a href="can-stache/expressions/helper.html" title="">Helper Expression</a>}</code>: <p>Calls a helper method
or function in the <a href="can-view-scope.html" title="Create a lookup node for keys.">can-view-scope</a> with specified arguments.</p>
</li>
...
work.</p>
<h2>KeyLookup and Call expressions</h2>
...
<p>Reminder: Sections will reset the current context to the value for which it is iterating.
...
<h2>Helper expression</h2>
<p>A helper like:</p>
<pre><code class="language-js">stache.registerHelper('countTo', function(number, options){
    var out = [];
    if(number &gt; 0) {
...
<p>Called with data like:</p>
<pre><code class="language-js">{number: 3}
</code></pre>
<p>Produces:</p>
<pre><code>&lt;p&gt; 1 2 3 &lt;/p&gt;
</code></pre>
...
<p>Produces:</p>
<pre><code>&lt;p&gt; Can't count to -5! &lt;/p&gt;
</code></pre>
<p>Notice how <code>options</code> has <code>.fn</code> and <code>.inverse</code>.</p>
