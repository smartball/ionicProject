...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-connect/cache-requests/cache-requests.js
	@module can-connect/cache-requests/cache-requests
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - cache-requests</title>
...
lists.">
	...
	<ul>
					...
					<li class="
           						...
           						">
						<a class="function"
							href="cache-requests.getListData.html"
							title="Only request data that isn&#x27;t already present in the cache.">
							getListData
						</a>
	...
	<ul>
					...
					<li class="
           						...
           						">
						<a class="function"
							href="cache-requests.getDiff.html"
							title="Compares the cached sets to the requested set and returns a description of what subset can be loaded from the
cache and what subset must be loaded from the base connection.">
					...
					<li class="
           						...
           						">
						<a class="function"
							href="cache-requests.getUnion.html"
							title="Create the requested data set, a union of the cached and un-cached data.">
							getUnion
						</a>
...
<div class="breadcrumb">
	...
	<li><a href="cache-requests.html">./cache-requests/</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>cache-requests</h1>
			<ul class="title-social">
				
				
				<li>
					<a class="button-link" href="//github.com/canjs/can-connect/edit/master/cache-requests/cache-requests.js#L7">Edit on GitHub</a>
				</li>
    ...
    <p>Cache response data and use it to prevent unnecessary future requests or make future requests smaller.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>cacheRequests( baseConnection )</code>
  </h2>
  <p>Provide an implementation of <a href="cache-requests.getListData.html" title="Only request data that isn't already present in the cache.">getListData</a> that uses set logic to
determine what data is already in the <a href="../base/base.cacheConnection.html" title="An underlying can-connect connection used when fetching data from a cache.">cache</a> and what data needs to be
loaded from the base connection.</p>
<p>It then gets data from the cache and the base connection (if needed), merges it, and returns it. Any data returned
from the base connection is added to the cache.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>baseConnection</b> <code>{Object}</code>: <p><code>can-connect</code> connection object that is having the <code>cache-requests</code> behavior added
on to it. Should already contain the behaviors that provide the <a href="../DataInterface.html" title="The most common raw data methods.">DataInterface</a>
(e.g <a href="../data/url/url.html" title="">data/url</a>). If the <code>connect</code> helper is used to build the connection, the behaviors will
automatically be ordered as required.</p>
</li>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{Object}</code>: <p>A <code>can-connect</code> connection containing the methods provided by <code>cache-requests</code>.</p>
</p>
  </div>
    ...
    <section class="body">
    <h2>Use</h2>
<p>Use <code>cache-requests</code> in combination with a cache like <a href="../data/memory-cache/memory-cache.html" title="Saves raw data in JavaScript memory that disappears when the page refreshes.">memory-cache</a> or
<a href="../data/localstorage-cache/localstorage-cache.html" title="Saves raw data in localStorage.">localstorage-cache</a>.  For example, to make it so response data is cached
in memory:</p>
<pre><code>var memoryCache = require(&quot;can-connect/data/memory-cache/&quot;);
...
<p>Now if today's todos are loaded:</p>
<pre><code>todoConnection.getListData({due: &quot;today&quot;});
</code></pre>
<p>And later, a subset of those todos are loaded:</p>
<pre><code>todoConnection.getListData({due: &quot;today&quot;, status: &quot;critical&quot;});
</code></pre>
<p>The subset will be created from the original request's data.</p>
<h2>Algebra Usage</h2>
<p><code>cache-requests</code> can also &quot;fill in&quot; the data the cache is missing if you provide it the necessary <a href="../../can-set.html" title="can-set is a utility for comparing sets that are represented by the parameters commonly passed to service requests.">set algebra</a>.</p>
<p>For example, if you requested paginated data like:</p>
<pre><code>todoConnection.getListData({start: 1, end: 10})
</code></pre>
<p>And then later requested:</p>
<pre><code>todoConnection.getListData({start: 1, end: 20})
</code></pre>
<p>... with the appropriate algebra configuration, <code>cache-requests</code> will only request <code>{start: 11, end: 20}</code>, merging
that response with the data already present in the cache.</p>
<p>That configuration looks like:</p>
<pre><code>var algebra = new set.Algebra( set.props.rangeInclusive(&quot;start&quot;,&quot;end&quot;) );
...
<p><strong>Note:</strong> <code>cacheConnection</code> shares the same algebra configuration as the primary connection.</p>
...
<script type="text/javascript">
  window.docObject = {"type":"module","name":"can-connect/cache-requests/cache-requests","parent":"can-connect.behaviors","src":{"line":6,"codeLine":99,"path":"node_modules/can-connect/cache-requests/cache-requests.js"},"description":"\nCache response data and use it to prevent unnecessary future requests or make future requests smaller.\n","title":"cache-requests","signatures":[{"code":"cacheRequests( baseConnection )","description":"\n\n  Provide an implementation of [can-connect/cache-requests/cache-requests.getListData] that uses set logic to\n  determine what data is already in the [can-connect/base/base.cacheConnection cache] and what data needs to be\n  loaded from the base connection.\n\n  It then gets data from the cache and the base connection (if needed), merges it, and returns it. Any data returned\n  from the base connection is added to the cache.\n","params":[{"types":[{"type":"Object","options":[]}],"name":"baseConnection","description":"`can-connect` connection object that is having the `cache-requests` behavior added\non to it. Should already contain the behaviors that provide the [can-connect/DataInterface]\n(e.g [can-connect/data/url/url]). If the `connect` helper is used to build the connection, the behaviors will\nautomatically be ordered as required.\n"}],"returns":{"types":[{"type":"Object","options":[]}],"description":"A `can-connect` connection containing the methods provided by `cache-requests`.\n\n"}}],"_curParam":{"types":[{"type":"Object","options":[]}],"name":"baseConnection","description":"`can-connect` connection object that is having the `cache-requests` behavior added\non to it. Should already contain the behaviors that provide the [can-connect/DataInterface]\n(e.g [can-connect/data/url/url]). If the `connect` helper is used to build the connection, the behaviors will\nautomatically be ordered as required.\n"},"_curReturn":{"types":[{"type":"Object","options":[]}],"description":"A `can-connect` connection containing the methods provided by `cache-requests`.\n\n"},"comment":" ","pathToRoot":"../../.."};
</script>
</article>
