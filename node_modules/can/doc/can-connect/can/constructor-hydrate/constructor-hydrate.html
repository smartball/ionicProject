...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-connect/can/constructor-hydrate/constructor-hydrate.js
	@module can-connect/can/constructor-hydrate/constructor-hydrate
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - constructor-hydrate</title>
...
<div class="breadcrumb">
	...
	<li><a href="constructor-hydrate.html">./can/constructor-hydrate/</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>constructor-hydrate</h1>
			<ul class="title-social">
				
				
				<li>
					<a class="button-link" href="//github.com/canjs/can-connect/edit/master/can/constructor-hydrate/constructor-hydrate.js">Edit on GitHub</a>
				</li>
    ...
    <p>Check the <a href="../../constructor/store/store.instanceStore.html" title="A mapping of instances keyed by their base.id.">instanceStore</a> when creating new instances of the connected
...
instances when instances are created outside of the <code>can-connect</code> connection.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>constructorHydrate( baseConnection )</code>
  </h2>
  <p>Overrides <a href="../../../can-define/map/map.html" title="Create observable objects.">DefineMaps</a>'s <code>setup</code> method and checks whether a newly created instance already
exists in the <a href="../../constructor/store/store.instanceStore.html" title="A mapping of instances keyed by their base.id.">instanceStore</a>. If an instance exists in the
<a href="../../constructor/store/store.instanceStore.html" title="A mapping of instances keyed by their base.id.">instanceStore</a> that instance will be returned instead of a new object.</p>
<p>This behavior expects to be used with the <a href="../../constructor/store/store.html" title="Adds support for keeping references to active lists and instances. Prevents different copies of an instance from
being used by the application at once. Allows other behaviors to look up instances currently active in the
application.">constructor/store</a> and <a href="../map/map.html" title="Integrate a can-connect connection with a DefineMap type.">can/map</a>
behaviors.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>baseConnection</b> <code>{Object}</code>: <p><code>can-connect</code> connection object that is having the <code>constructor-hydrate</code> behavior added
on to it.</p>
</li>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{Object}</code>: <p>a <code>can-connect</code> connection containing the methods provided by <code>constructor-hydrate</code>.</p>
</p>
  </div>
    ...
    <section class="body">
    <h2>Use</h2>
<p>During all initializations of the connection <a href="../map/map._Map.html" title="Specify the type of the DefineMap that should be instantiated by the connection.">Map type</a>, if an id property exists then
the <code>constructor-hydrate</code> behavior will check the connection <a href="../../constructor/store/store.instanceStore.html" title="A mapping of instances keyed by their base.id.">instanceStore</a> for
a matching instance. If one is found it will be returned instead of a new object.</p>
<p>For example, this behavior is useful if <a href="../../../can-define.types.propDefinition.html" title="Defines the type, initial value, and get, set, and serialize behavior for an
observable property.  These behaviors can be specified with as an Object, String,
Constructor function, Array, a getter expression, or setter expression."><code>Type</code> converters</a> of
<a href="../../../can-define/map/map.html" title="Create observable objects.">DefineMap</a> are used in your app.</p>
<p>Let's say we have a page state type, two properties of which are of type <code>Student</code>:</p>
<pre><code class="language-js">var myPage = new (DefineMap.extend({
  student: { Type: Student },
  teamLead: { Type: Student },
  loadTeamLead: function() {...}
}));
</code></pre>
<p>The type <code>Student</code> is a DefineMap that has had a <code>can-connect</code> connection attached:</p>
...
<p>Now lets say your page loads <code>myPage.student</code> via the connection using <code>Student.get()</code>, and then it gets data for
<code>teamLead</code> without the connection via <code>myPage.loadTeamLead()</code>. In this example it so happens that the team lead is
the same person as the student:</p>
<pre><code class="language-js">Student.get({id: 1}).then(function(person) {
...
<p>Without <code>constructor-hydrate</code> we would end up with two different instances of <code>Student</code> with the same id. Additionally,
<code>teamLead</code> would not be an instance that is stored in the connection's <code>instanceStore</code>, and thus would not benefit
from the real-time updates offered by the <a href="../../real-time/real-time.html" title="Update lists to include or exclude instances based
on set logic.">real-time</a> behavior.</p>
<p><code>constructor-hydrate</code> solves this problem by checking <code>instanceStore</code> before creating a new instance. So, in our
example it will return the existing instance from <code>myPage.loadTeamLead()</code>. Now both <code>myPage.student</code> and
<code>myPage.teamLead</code> are referencing the same instance:</p>
<pre><code class="language-js">var instanceStore = Student.connection.instanceStore;
...
<script type="text/javascript">
  window.docObject = {"name":"can-connect/can/constructor-hydrate/constructor-hydrate","type":"module","parent":"can-connect.behaviors","src":{"line":0,"codeLine":93,"path":"node_modules/can-connect/can/constructor-hydrate/constructor-hydrate.js"},"description":"\nCheck the [can-connect/constructor/store/store.instanceStore] when creating new instances of the connected\n[can-connect/can/map/map._Map] type. Return an existing instance if a match is found. Prevents duplication of\ninstances when instances are created outside of the `can-connect` connection.\n","title":"constructor-hydrate","types":[{"type":"connect.Behavior"}],"signatures":[{"code":"constructorHydrate( baseConnection )","description":"\n\nOverrides [can-define/map/map DefineMaps]'s `setup` method and checks whether a newly created instance already\nexists in the [can-connect/constructor/store/store.instanceStore]. If an instance exists in the\n[can-connect/constructor/store/store.instanceStore] that instance will be returned instead of a new object.\n\nThis behavior expects to be used with the [can-connect/constructor/store/store] and [can-connect/can/map/map can/map]\nbehaviors.\n","params":[{"types":[{"type":"Object","options":[]}],"name":"baseConnection","description":"`can-connect` connection object that is having the `constructor-hydrate` behavior added\n  on to it.\n"}],"returns":{"types":[{"type":"Object","options":[]}],"description":"a `can-connect` connection containing the methods provided by `constructor-hydrate`.\n"}}],"_curParam":{"types":[{"type":"Object","options":[]}],"name":"baseConnection","description":"`can-connect` connection object that is having the `constructor-hydrate` behavior added\n  on to it.\n"},"_curReturn":{"types":[{"type":"Object","options":[]}],"description":"a `can-connect` connection containing the methods provided by `constructor-hydrate`.\n"},"comment":" ","pathToRoot":"../../../.."};
</script>
</article>
