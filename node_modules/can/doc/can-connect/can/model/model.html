...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-connect/can/model/model.md
	@module can-connect/can/model/model
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - can-connect/can/model/model</title>
...
<div class="breadcrumb">
	...
	<li><a href="model.html">./can/model/</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>can-connect/can/model/model</h1>
			<ul class="title-social">
				
				
				<li>
					<a class="button-link" href="//github.com/canjs/can-connect/edit/master/can/model/model.md">Edit on GitHub</a>
				</li>
    ...
    <p>Exports a constructor that works very similar to <a href="http://canjs.com/docs/can.Model.html">can.Model</a>.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>Model.extend( static, prototype )</code>
  </h2>
  <p>Defines a <a href="../../../can-map.html" title="Create observable objects.">can-map</a> that has almost all of the functionality of
<code>can.Model</code>.</p>
    ...
    <section class="body">
    <h2>Use</h2>
<p><code>can-connect/can/model</code> is for backwards compatibility
with <code>can.Model</code> so that developers can migrate
to <code>can-connect</code> without having to rewrite their models immediately.</p>
<p>However, use of <code>can.Model</code> will be deprecated in CanJS 3.0. Instead of extending <code>can.Model</code>,
extend <a href="../../../can-map.html" title="Create observable objects.">can-map</a> and <a href="../../../can-list.html" title="">can-list</a> and use the <a href="../map/map.html" title="Integrate a can-connect connection with a DefineMap type.">can/map</a> behavior to connect your Map and List to a connection:</p>
...
<p>Or, use the <a href="../super-map/super-map.html" title="Create connection with many of the best behaviors in can-connect and hook it up to
a map.">can-connect/can/super-map/super-map</a> function to create a connection with the &quot;super&quot; behaviors:</p>
<pre><code>var todoConnection = superMap({
  idProp: &quot;_id&quot;,
  Map: Todo,
  List: TodoList,
  url: &quot;/services/todos&quot;,
...
<p>For your legacy code, you should just need to import &quot;can-connect/can/model/&quot; instead of &quot;can/model/&quot; like:</p>
<pre><code class="language-js">import Model from &quot;can-connect/can/model/&quot;;
...
<h2>Upgrading can.Models to can-connect</h2>
<p>This section walks through making the necessary changes to upgrade a legacy can.Model to use <code>can-connect</code> and its
behaviors.  We'll convert a <code>can.Model</code> and <code>can.Model.List</code> that looks like:</p>
...
<p>Converting this to use <code>can-connect</code> looks like:</p>
<pre><code class="language-js">var CanMap = require(&quot;can-map&quot;),
...
<p>Instead of <code>Todo.findAll</code> and <code>Todo.findOne</code>, use <code>Todo.getList</code> and <code>Todo.get</code>.</p>
<p>Lets break this down in the following sections.</p>
<h3>Defining the Map and List</h3>
<p>The first step is to pull out the parts of the Model and Model.List that define the
observable Map and List's behavior into a <a href="../../../can-map.html" title="Create observable objects.">can-map</a> and <a href="../../../can-list.html" title="">can-list</a>:</p>
<pre><code class="language-js">var CanMap = require(&quot;can-map&quot;),
    CanList = require(&quot;can-list&quot;);
...
<p>One of the main advantageous of <code>can-connect</code> is that it lets separate persistence behavior
from property behavior.</p>
<h3>Connecting the Map and List to behaviors</h3>
<p>The next step is to connect the Map and List to the right behaviors.  The following
adds behaviors with similar functionality to legacy <code>can.Map</code> and uses the <a href="../map/map.html" title="Integrate a can-connect connection with a DefineMap type.">can/map</a> behavior
(which makes use of <a href="../../constructor/constructor.html" title="Adds an interface to interact with custom types via the connection instead of plain Objects and Arrays.">constructor</a>) to connect the connection to the provided Map and List types:</p>
<pre><code>connect([
    require(&quot;can-connect/data/url/url&quot;),
    require(&quot;can-connect/data/parse/parse&quot;),
    require(&quot;can-connect/constructor/constructor&quot;),
...
<h3>Connecting to urls</h3>
<p>The <a href="../../data/url/url.html" title="">data/url</a> behavior supports CRUDing data from urls.  It can be configured like:</p>
<pre><code>connect([&quot;data-url&quot;, ...],
  {
    ...
    url: {
      resource: &quot;/&quot;,
...
<h3>Correcting response data</h3>
<p>The <a href="../../data/parse/parse.html" title="Extract response data into a format needed for other extensions.">can-connect/data/parse/parse</a> behavior supports correcting response data.  It can be configured like:</p>
<pre><code>connect([..., &quot;data-parse&quot;, ...],
  {
    ...
    parseListData: function(data){
      return data.todos;
...
<h3>Specifying the id</h3>
<p>The id of a model is used in a variety of ways.  It's part of the <a href="../../base/base.html" title="The first behavior added to every can-connect connection. Provides methods to uniquely identify instances and
lists.">base</a> behavior
added to every connection.  You can customize which property represents the id with <a href="../../base/base.idProp.html" title="Specifies the property that uniquely identifies an instance.">idProp</a>.</p>
<pre><code>connect([...],
  {
    idProp: &quot;_id&quot;
  });
...
<p>The <a href="../map/map.html" title="Integrate a can-connect connection with a DefineMap type.">can/map</a> behavior adds a <code>getList</code> and <code>get</code> method to the <code>Map</code> option.  Use them in
place of <code>findAll</code> and <code>findOne</code>:</p>
<pre><code>Todo.findAll({}).then(function(todosList){ ... });
Todo.findOne({id: 5}).then(function(todo){ ... });
</code></pre>
<h2>Differences from can.Model</h2>
<p>Model's produced from <code>can-connect/can/model</code>:</p>
<ul>
<li>Do not support <code>makeFindAll</code> or <code>makeFindOne</code>.  If your legacy code did this, you can probably add it as a custom behavior.</li>
<li>Has the instance store at <code>Model.store</code>, but items should be retrieved like <code>Model.store.get(id)</code>.</li>
<li>Should not use <code>Model.models</code> or <code>Model.model</code> to correct Ajax data and should instead use <code>Models.parseModel</code> and <code>Model.parseModels</code>.</li>
<li>Uses a Promise polyfill, not jQuery's promises.</li>
</ul>
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-connect/can/model/model.md"},"description":"Exports a constructor that works very similar to [can.Model](http://canjs.com/docs/can.Model.html). \n","type":"module","title":"","name":"can-connect/can/model/model","parent":"can-connect.modules","signatures":[{"code":"Model.extend( static, prototype )","description":"\n\n  Defines a [can-map] that has almost all of the functionality of\n  `can.Model`.\n","params":[]}],"comment":" ","pathToRoot":"../../../.."};
</script>
</article>
