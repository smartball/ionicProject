...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-connect/can/ref/ref.js
	@module can-connect/can/ref/ref
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - can/ref</title>
...
instances when instances are created outside of the can-connect connection.">
	...
	<ul>
					...
					<li class="
           						...
           						">
						<a class="property"
							href="ref.Map.Ref.html"
							title="A reference type with instanceRef.value primed to return an existing instance of the
map._Map type, if available, or lazy load an instance upon accessing instanceRef.value.">
							Map.Ref
						</a>
...
<div class="breadcrumb">
	...
	<li><a href="ref.html">./can/ref/</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>can/ref</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-connect/edit/master/can/ref/ref.js">Edit on GitHub</a>
				</li>
    ...
    <p>Handle references to instances in the data returned by the server. Allows several means of loading referenced instances, determined on-the-fly.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>canRef( baseConnection )</code>
  </h2>
  <p>Adds a reference type to <a href="../map/map._Map.html" title="Specify the type of the DefineMap that should be instantiated by the connection."><code>connection.Map</code></a> that loads the related type or holds onto
an existing one. This handles circular references and loads relevant data as needed. The reference type can be loaded
by:</p>
<ul>
<li>it's data being included in the response for the referencing instance</li>
<li>having an existing instance available in the <a href="../../constructor/store/store.instanceStore.html" title="A mapping of instances keyed by their base.id.">instanceStore</a></li>
<li>lazy loading via the connection for the reference type</li>
</ul>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>baseConnection</b> <code>{Object}</code>: <p><code>can-connect</code> connection object that is having the <code>can/ref</code> behavior added on to it.
Expects the <a href="../map/map.html" title="Integrate a can-connect connection with a DefineMap type.">can/map</a> behavior to already be added to this base connection. If the <code>connect</code> helper
is used to build the connection, the behaviors will automatically be ordered as required.</p>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{Object}</code>: <p>a connection with the <a href="../map/map._Map.html" title="Specify the type of the DefineMap that should be instantiated by the connection."><code>Map</code></a> having the reference type property
(<code>Map.Ref.type</code>) created by <code>can/ref</code>.</p>
</p>
  </div>
    ...
    <section class="body">
    <h2>Use</h2>
<p><code>can/ref</code> is useful when the server might return either a reference to
a value or the value itself.  For example, in a MongoDB setup,
a request like <code>GET /game/5</code> might return:</p>
<pre><code>{
  id: 5,
  teamRef: 7,
  score: 21
...
<p>But a request like <code>GET /game/5?$populate=teamRef</code> might return:</p>
<pre><code>{
  id: 5,
...
<p><code>can/ref</code> can handle this ambiguity and even make lazy loading possible.</p>
<p>To use <code>can/ref</code>, first create a Map and a connection for the referenced type:</p>
<pre><code>var Team = DefineMap.extend({
  id: 'string'
});
...
<p>The connection is necessary because it creates an instance store which will
hold instances of <code>Team</code> that the <code>Team.Ref</code> type will be able to access.</p>
<p>Now we can create a reference to the Team within a Game map and the Game's connection:</p>
<pre><code>var Game = DefineMap.extend({
  id: 'string',
...
<p>Now, <code>teamRef</code> is a <a href="ref.Map.Ref.html" title="A reference type with instanceRef.value primed to return an existing instance of the
map._Map type, if available, or lazy load an instance upon accessing instanceRef.value.">Map.Ref</a> type, which will
house the id of the reference no matter how the server returns data, e.g.
<code>game.teamRef.id</code>.</p>
<p>For example, without populating the team data:</p>
<pre><code>Game.get({id: 5}).then(function(game){
  game.teamRef.id //-&gt; 7
});
</code></pre>
<p>With populating the team data:</p>
<pre><code>Game.get({id: 5, $populate: &quot;teamRef&quot;}).then(function(game){
  game.teamRef.id //-&gt; 7
});
</code></pre>
<p>The values of other properties and methods on the <a href="ref.Map.Ref.html" title="A reference type with instanceRef.value primed to return an existing instance of the
map._Map type, if available, or lazy load an instance upon accessing instanceRef.value.">Map.Ref</a> type
are determined by if the reference was populated or the referenced item already exists
in the <a href="../../constructor/store/store.instanceStore.html" title="A mapping of instances keyed by their base.id.">instanceStore</a>.</p>
...
<p>Or, it will be populated if that instance had been loaded through another means and
it’s in the instance store:</p>
<pre><code>Team.get({id: 7}).then(function(team){
...
<p><code>value</code> is an <a href="../../../can-define.types.get.html" title="Specify what happens when a certain property is read on a map. get functions
work like a can-compute and automatically update themselves when a dependent
observable value is changed.">asynchronous getter</a>, which means that even if
the referenced value isn't populated or loaded through the store, it can be lazy loaded. This
is generally most useful in a template.</p>
<p>The following will make an initial request for game <code>5</code>, but when the template
tried to read and listen to <code>game.teamRef.value.name</code>, a request for team <code>7</code>
will be made.</p>
<pre><code>var template = stache(&quot;{{game.teamRef.value.name}} scored {{game.score}} points&quot;);
Game.get({id: 5}).then(function(game){
  template({game: game});
});
</code></pre>
...
<script type="text/javascript">
  window.docObject = {"name":"can-connect/can/ref/ref","type":"module","parent":"can-connect.behaviors","src":{"line":0,"codeLine":155,"path":"node_modules/can-connect/can/ref/ref.js"},"description":"Handle references to instances in the data returned by the server. Allows several means of loading referenced instances, determined on-the-fly.\n\n","title":"can/ref","types":[{"type":"connect.Behavior"}],"signatures":[{"code":"canRef( baseConnection )","description":"\n\nAdds a reference type to [can-connect/can/map/map._Map `connection.Map`] that loads the related type or holds onto\nan existing one. This handles circular references and loads relevant data as needed. The reference type can be loaded\nby:\n- it's data being included in the response for the referencing instance\n- having an existing instance available in the [can-connect/constructor/store/store.instanceStore]\n- lazy loading via the connection for the reference type\n","params":[{"types":[{"type":"Object","options":[]}],"name":"baseConnection","description":"`can-connect` connection object that is having the `can/ref` behavior added on to it.\nExpects the [can-connect/can/map/map] behavior to already be added to this base connection. If the `connect` helper\nis used to build the connection, the behaviors will automatically be ordered as required.\n"}],"returns":{"types":[{"type":"Object","options":[]}],"description":"a connection with the [can-connect/can/map/map._Map `Map`] having the reference type property\n(`Map.Ref.type`) created by `can/ref`.\n"}}],"_curParam":{"types":[{"type":"Object","options":[]}],"name":"baseConnection","description":"`can-connect` connection object that is having the `can/ref` behavior added on to it.\nExpects the [can-connect/can/map/map] behavior to already be added to this base connection. If the `connect` helper\nis used to build the connection, the behaviors will automatically be ordered as required.\n"},"_curReturn":{"types":[{"type":"Object","options":[]}],"description":"a connection with the [can-connect/can/map/map._Map `Map`] having the reference type property\n(`Map.Ref.type`) created by `can/ref`.\n"},"comment":" ","pathToRoot":"../../../.."};
</script>
</article>
