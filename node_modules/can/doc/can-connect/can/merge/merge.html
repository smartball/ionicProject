...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-connect/can/merge/merge.md
	@module can-connect/can/merge/merge
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - can/merge</title>
...
instances when instances are created outside of the can-connect connection.">
	...
	<ul>
					...
					<li class="
           						...
           						">
						<a class="function"
							href="merge.createdInstance.html"
							title="Makes minimal updates to an instance&#x27;s properties and its nested properties using map-deep-merge.">
							createdInstance
						</a>
					...
					<li class="
           						...
           						">
						<a class="function"
							href="merge.destroyedInstance.html"
							title="Makes minimal updates to an instance&#x27;s properties and its nested properties using map-deep-merge.">
							destroyedInstance
						</a>
					...
					<li class="
           						...
           						">
						<a class="function"
							href="merge.updatedInstance.html"
							title="Makes minimal updates to an instance&#x27;s properties and its nested properties using map-deep-merge.">
							updatedInstance
						</a>
					...
					<li class="
           						...
           						">
						<a class="function"
							href="merge.updatedList.html"
							title="Makes minimal updates to an list&#x27;s items and those items&#x27; nested properties using map-deep-merge.">
							updatedList
						</a>
...
<div class="breadcrumb">
	...
	<li><a href="merge.html">./can/merge/</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>can/merge</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-connect/edit/master/can/merge/merge.md">Edit on GitHub</a>
				</li>
    ...
    <p>Minimally update nested data structures with the response from the server.</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>canMergeBehavior( baseConnection )</code>
  </h2>
  <p>Overwrites <a href="../map/map.html" title="Integrate a can-connect connection with a DefineMap type.">can/map</a>'s instance callbacks so they use <a href="../../helpers/map-deep-merge.html" title="Perform a smart merge of deeply nested maps and lists.">map-deep-merge</a>.
<a href="../../helpers/map-deep-merge.html" title="Perform a smart merge of deeply nested maps and lists.">map-deep-merge</a> is able to make minimal changes to the nested properties of <a href="../../../can-define.html" title="Exports the define method that defines observable properties and their behavior on a prototype object.">can-define</a> instances
and lists given raw data.
E.g:</p>
<pre><code class="language-js">var existingStudent;
...
<p>To use <code>can/merge</code>, the connection's <a href="../map/map._Map.html" title="Specify the type of the DefineMap that should be instantiated by the connection.">Map</a>, <a href="../map/map._List.html" title="Specify the type of the DefineList that should be instantiated by the connection.">List</a> and any of their
nested types must be properly configured.  That configuration is discussed in the
<a href="merge.html#Use" title="Minimally update nested data structures with the response from the server.">&quot;Use&quot; section</a> below.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>baseConnection</b> <code>{Object}</code>: <p><code>can-connect</code> connection object that is having the <code>can/merge</code> behavior added on to it. Expects
the <a href="../map/map.html" title="Integrate a can-connect connection with a DefineMap type.">can/map</a> behavior to already be added to this base connection. If the <code>connect</code> helper
is used to build the connection, the behaviors will automatically be ordered as required.</p>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{Object}</code>: <p>a <code>can-connect</code> connection containing the methods provided by <code>can/merge</code>.</p>
</p>
  </div>
    ...
    <section class="body">
    <h2>Use</h2>
<p>To use the <code>can/merge</code> behavior, you have to:</p>
<ol>
<li>Add the behavior after <a href="../map/map.html" title="Integrate a can-connect connection with a DefineMap type.">can/map</a>, and</li>
<li>Make sure all types, especially <a href="../map/map._List.html" title="Specify the type of the DefineList that should be instantiated by the connection.">List</a> type is properly configured.</li>
</ol>
<p>Adding the <code>can/merge</code> behavior after <a href="../map/map.html" title="Integrate a can-connect connection with a DefineMap type.">can/map</a> is pretty straightforward.
When you create a custom connection, create it as follows:</p>
<pre><code class="language-js">var canMergeBehavior = require(&quot;can-connect/can/merge/merge&quot;);
...
<p>For <a href="../../helpers/map-deep-merge.html" title="Perform a smart merge of deeply nested maps and lists.">map-deep-merge</a> to merge correctly, it needs to know how to uniquely identify an instance and
be able to convert raw data to instances and lists.
<code>map-deep-merge</code> looks for this configuration on the <code>.algebra</code> and <code>.connection</code> properties of the
<a href="../../../can-define.types.TypeConstructor.html" title="Provides a constructor function to be used to convert any set value into an appropriate
value.">Type</a> setting on <a href="../../../can-define.html" title="Exports the define method that defines observable properties and their behavior on a prototype object.">can-define</a> types.</p>
<p>This is more easily understood in an example.
If the <code>ClassRoom</code> has a <code>students</code> property that is a list of <code>Student</code> instances like:</p>
<pre><code class="language-js">var ClassRoom = DefineMap.extend({
    students: Student.List
});
</code></pre>
<p>To be able to uniquely identify <code>Student</code> instances within that list, make sure <code>Student</code> has an <code>algebra</code> property
that is configured with the identifier property:</p>
<pre><code class="language-js">Student = DefineMap.extend({ ... });

Student.algebra = new set.Algebra(set.props.id(&quot;_id&quot;))
</code></pre>
<p>Also make sure that <code>Student.List</code> points its <a href="../../../can-define/list/list.prototype.wildcardItems.html" title="Define default behavior for items in the list.">#</a> definition to <code>Student</code>
like the following:</p>
<pre><code class="language-js">Student.List = DefineList.extend({
    &quot;#&quot;: Student
});
</code></pre>
<p><strong>Note:</strong> the typical method used to create a <code>Student</code> is <code>new Student(props)</code>.
However, if <code>Student</code>s have a <code>.connection</code>, <a href="../../helpers/map-deep-merge.html" title="Perform a smart merge of deeply nested maps and lists.">map-deep-merge</a> will use
<code>Student.connection.<a href="../../constructor/constructor.hydrateInstance.html" title="Produce a typed object containing the provided raw data.">hydrateInstance</a>(props)</code>.
This is useful if <code>Student</code>s should be looked up in the connection <a href="../../constructor/store/store.instanceStore.html" title="A mapping of instances keyed by their base.id.">instanceStore</a>.</p>
<p>For example, <code>Student</code> might have a connection that has an <a href="../../constructor/store/store.instanceStore.html" title="A mapping of instances keyed by their base.id.">instanceStore</a>, like:</p>
<pre><code class="language-js">Student.connection = baseMap({
    Map: Student,
    List: Student.List,
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-connect/can/merge/merge.md"},"description":"\nMinimally update nested data structures with the response from the server.\n","type":"module","title":"can/merge","types":[{"type":"connect.Behavior"}],"name":"can-connect/can/merge/merge","parent":"can-connect.behaviors","signatures":[{"code":"canMergeBehavior( baseConnection )","description":"\n\nOverwrites [can-connect/can/map/map]'s instance callbacks so they use [can-connect/helpers/map-deep-merge].\n[can-connect/helpers/map-deep-merge] is able to make minimal changes to the nested properties of [can-define] instances \nand lists given raw data. \nE.g:\n\n```js\nvar existingStudent;\nvar classroom = ClassRoom.get({id: 505}).then(function(instance) {\n  instance.id; // 505\n  instance.students[0].id; // 15\n  instance.students[0].name; // 'Samantha Jones'\n  existingStudent = instance;\n});\n\n... // later in the program new information for the classroom is retrieved\n\nClassRoom.get({id:505}).then(function(instance) {\n    instance.id; // 505\n    instance.students[0].id; // 15\n    instance.students[0].name; // 'Samantha Jones-Baker'\n    \n    // true, if can-merge behavior is used. \n    // a new nested instance isn't created, instead it was updated with the changed fields\n    existingStudent === instance.students[0]; \n});\n\n```\n\nTo use `can/merge`, the connection's [can-connect/can/map/map._Map], [can-connect/can/map/map._List] and any of their \nnested types must be properly configured.  That configuration is discussed in the \n[can-connect/can/merge/merge#Use \"Use\" section] below.\n","params":[{"types":[{"type":"Object","options":[]}],"name":"baseConnection","description":"`can-connect` connection object that is having the `can/merge` behavior added on to it. Expects\nthe [can-connect/can/map/map] behavior to already be added to this base connection. If the `connect` helper \nis used to build the connection, the behaviors will automatically be ordered as required.\n"}],"returns":{"types":[{"type":"Object","options":[]}],"description":"a `can-connect` connection containing the methods provided by `can/merge`.\n"}}],"_curParam":{"types":[{"type":"Object","options":[]}],"name":"baseConnection","description":"`can-connect` connection object that is having the `can/merge` behavior added on to it. Expects\nthe [can-connect/can/map/map] behavior to already be added to this base connection. If the `connect` helper \nis used to build the connection, the behaviors will automatically be ordered as required.\n"},"_curReturn":{"types":[{"type":"Object","options":[]}],"description":"a `can-connect` connection containing the methods provided by `can/merge`.\n"},"comment":" ","pathToRoot":"../../../.."};
</script>
</article>
