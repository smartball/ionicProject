...
<!--####################################################################
	...
	@function can-connect/constructor/constructor.save
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - save</title>
...
<div class="breadcrumb">
	...
	<li><a href="constructor.save.html">save</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>save</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-connect/edit/master/constructor/constructor.js#L280">Edit on GitHub</a>
				</li>
    ...
    <p>Create or update an instance on the connection data source</p>
...
<div class="signature">
  <h2 class="signature-title">
    <code>connection.save( instance )</code>
  </h2>
  <p>First checks if the instance has an <a href="../base/base.id.html" title="Uniquely identify an instance or raw instance data.">id</a> or not.  If it has an id, the instance will be
updated; otherwise, it will be created.</p>
<p>When creating an instance, the instance is added to the <a href="constructor.cidStore.html" title="Temporarily hold references to new instances via their cid while they are undergoing creation.">cidStore</a>, and its
<a href="constructor.serializeInstance.html" title="Generate the serialized form of a typed instance.">serialized data</a> is passed to
<a href="../connection.createData.html" title="Creates a new record in the connection.">createData</a>.  If <code>createData</code>'s promise resolves to anything other than <code>undefined</code>,
<a href="constructor.createdInstance.html" title="A method run whenever a new instance has been saved to the data source. Updates the instance with response data.">createdInstance</a> is called with that data.</p>
<p>When updating an instance, its <a href="constructor.serializeInstance.html" title="Generate the serialized form of a typed instance.">serialized data</a> is
passed to <a href="../connection.updateData.html" title="Updates a record in the collection.">updateData</a>. If <code>updateData</code>'s promise resolves to anything other than
<code>undefined</code>, <a href="constructor.updatedInstance.html" title="A method run whenever an existing instance has been saved to the data source. Overwrites the instance with response
data.">updatedInstance</a> is called with that data.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>instance</b> <code>{<a href="../Instance.html" title="An instance of some JavaScript type.">Instance</a>}</code>: <p>the instance to create or save</p>
</li>
  ...
  <div class="returns">
    <h3 class="returns-title">Returns</h3>
     <p> <code>{Promise&lt;<a href="../Instance.html" title="An instance of some JavaScript type.">Instance</a>&gt;}</code>: <p><code>Promise</code> resolving to the same instance that was passed to <code>save</code></p>
</p>
  </div>
    ...
    <section class="body">
    <h2>Use</h2>
<p>To use <code>save</code> to create an instance, create a connection, then an instance, and call <code>.save()</code> on it:</p>
...
<p><code>.save(todo)</code> above will call <a href="../data/url/url.createData.html" title=""><code>createData</code></a> on the <a href="../data/url/url.html" title="">data/url</a>
behavior, which will make an HTTP POST request to <code>/todos</code> with the serialized <code>todo</code> data.  The server response
data may look something like:</p>
<pre><code>{
 id: 5,
 ownerId: 9
}
</code></pre>
<p>That data will be passed to <a href="constructor.createdInstance.html" title="A method run whenever a new instance has been saved to the data source. Updates the instance with response data.">createdInstance</a> which by default
adds those properties to <code>todo</code>, resulting in <code>todo</code> looking like:</p>
<pre><code>{
 name: &quot;do dishes&quot;,
 id: 5,
 ownerId: 9
...
<p>As an example of updating an instance, change a property on <code>todo</code> and call <code>.save()</code> again:</p>
<pre><code>// Change a property
todo.name = &quot;Do dishes now!!&quot;;

// Call .save()
todoConnection.save(todo)
</code></pre>
<p>The <code>.save(todo)</code> above will call <a href="../data/url/url.updateData.html" title=""><code>updateData</code></a> on the
<a href="../data/url/url.html" title="">data/url</a> behavior, which will make an HTTP PUT request to <code>/todos</code> with the serialized <code>todo</code>
data.</p>
<p>A successful server response body should look something like:</p>
<pre><code>{
 name: &quot;Do dishes now!!&quot;,
 id: 5,
 ownerId: 9
...
<p>This data will be passed to <a href="constructor.updatedInstance.html" title="A method run whenever an existing instance has been saved to the data source. Overwrites the instance with response
data.">updatedInstance</a> which by default sets
all of <code>todo</code>'s properties to look like the response data, even removing properties that are missing from the
response data.</p>
...
<script type="text/javascript">
  window.docObject = {"type":"function","name":"can-connect/constructor/constructor.save","parent":"can-connect/constructor/constructor.crud","src":{"line":279,"codeLine":374,"path":"node_modules/can-connect/constructor/constructor.js"},"description":"Create or update an instance on the connection data source \n","title":"save","signatures":[{"code":"connection.save( instance )","description":"\n\n  First checks if the instance has an [can-connect/base/base.id] or not.  If it has an id, the instance will be\n  updated; otherwise, it will be created.\n\n  When creating an instance, the instance is added to the [can-connect/constructor/constructor.cidStore], and its\n  [can-connect/constructor/constructor.serializeInstance serialized data] is passed to\n  [can-connect/connection.createData].  If `createData`'s promise resolves to anything other than `undefined`,\n  [can-connect/constructor/constructor.createdInstance] is called with that data.\n\n  When updating an instance, its [can-connect/constructor/constructor.serializeInstance serialized data] is\n  passed to [can-connect/connection.updateData]. If `updateData`'s promise resolves to anything other than\n  `undefined`, [can-connect/constructor/constructor.updatedInstance] is called with that data.\n","params":[{"types":[{"type":"can-connect/Instance"}],"name":"instance","description":"the instance to create or save\n"}],"returns":{"types":[{"type":"Promise","template":[{"types":[{"type":"can-connect/Instance"}]}]}],"description":"`Promise` resolving to the same instance that was passed to `save`\n"}}],"_curParam":{"types":[{"type":"can-connect/Instance"}],"name":"instance","description":"the instance to create or save\n"},"_curReturn":{"types":[{"type":"Promise","template":[{"types":[{"type":"can-connect/Instance"}]}]}],"description":"`Promise` resolving to the same instance that was passed to `save`\n"},"comment":" ","pathToRoot":"../../.."};
</script>
</article>
