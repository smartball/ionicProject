...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-stache/docs/keys/at.md
	@typedef can-stache/keys/at
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - @at</title>
...
<div class="breadcrumb">
  ...
  <section class="title">
	<div class="page-type">
		<h1>@at</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-stache/edit/master/docs/keys/at.md">Edit on GitHub</a>
				</li>
    ...
    <p>Return whatever value is at a key, regardless
...
<div class="signature">
  ...
  <p>Lookup a <code>key</code> value in the scope and return whatever is there.</p>
<pre><code>&lt;paginator {next}=&quot;@key&quot;/&gt;
...
<div class="signature">
  ...
  <p>Lookup <code>prop</code> property on <code>key</code> and return whatever is there.</p>
<pre><code>&lt;grid {get-data}=&quot;key@prop&quot;/&gt;
&lt;grid {get-data}=&quot;Todo@getList&quot;/&gt;
    ...
    <section class="body">
    <p>The default behavior in Stache lookups is to execute functions to get their return values.  <code>@</code> before a token in a path <code>@A.B</code> prevents A from being called for its return value (so 'B' is evaluated from the properties of A itself). Likewise, <code>@</code> elsewhere in a path like <code>A@B</code> prevents B from being called for its return value and returns whatever B itself is on the return from A.</p>
<h2>Use</h2>
...
<p>If the AT key is used at the start of a key like:</p>
<pre><code>{{method(@key)}}
</code></pre>
...
<p>The following illustrates what <code>@some.key</code> would return because <code>some</code> will not be executed if it's a function:</p>
<pre><code>// A non-observable JS object with intermediate functions:
{some: function(){ return {key: &quot;value&quot;}}}
   //-&gt; undefined
...
<p>The AT key can be used multiple times within a value lookup expression like:</p>
<pre><code>{{method(models@Todo@getList)}}
</code></pre>
...
<script type="text/javascript">
  window.docObject = {"src":{"path":"node_modules/can-stache/docs/keys/at.md"},"description":"\nReturn whatever value is at a key, regardless\nif it's a function or a compute.\n","type":"typedef","title":"@at","types":[{"type":"String"}],"name":"can-stache/keys/at","parent":"can-stache/keys","signatures":[{"code":"@key","description":"\n\nLookup a `key` value in the scope and return whatever is there.\n\n```\n<paginator {next}=\"@key\"/>\n<paginator {next}=\"@loadNextItem\"/>\n```\n","params":[]},{"code":"key@prop","description":"\n\nLookup `prop` property on `key` and return whatever is there.\n\n```\n<grid {get-data}=\"key@prop\"/>\n<grid {get-data}=\"Todo@getList\"/>\n```\n","params":[]}],"comment":" ","pathToRoot":"../../.."};
</script>
</article>
