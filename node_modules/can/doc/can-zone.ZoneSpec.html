...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-zone/docs/ZoneSpec.md
	@typedef can-zone.ZoneSpec
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - ZoneSpec</title>
...
<div class="breadcrumb">
	...
	<li><a href="can-zone.ZoneSpec.html">ZoneSpec</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>ZoneSpec</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-zone/edit/master/docs/ZoneSpec.md">Edit on GitHub</a>
				</li>
    ...
    <p>A ZoneSpec is the way you tap into the lifecycle hooks of a <a href="can-zone.html" title="A library that tracks asynchronous activity and lets you know when it has completed. Useful when you need to call a function and wait for all async behavior to complete, such as when performing server-side rendering.">Zone</a>. The hooks are described below.</p>
<p>Using these hooks you can do things like create timers and override global variables that will change the <em>shape</em> of code that runs within the Zone.</p>
...
<div class="signature">
  ...
  <h3 class="options-title">Options</h3>
    <ul>
      ...
      <li>
        <b>created</b> <code>{function}</code>:
        <p>Called when the zone is first created, after all ZoneSpecs have been parsed. this is useful if you need to do setup behavior that covers the entire zone lifecycle.</p>
<pre><code class="language-js">new Zone({
    created: function(){
      ...
      <li>
        <b>beforeRun</b> <code>{function}</code>:
        <p>Called immediately before the <a href="can-zone.prototype.run.html" title="">run</a> function is called.</p>
...
zone.run(function() { ... });
      ...
      <li>
        <b>afterRun</b> <code>{function}</code>:
        <p>Called immediately after the <a href="can-zone.prototype.run.html" title="">run</a> function is called. This hook is useful for any cleanup that might need to be done after the run function is called, but before the zone's promise is resolved. You might use this if the promise is not waited on before performing some action.</p>
<pre><code class="language-js">require(&quot;http&quot;).createServer(function(req, res){
    ...
    res.end();
      ...
      <li>
        <b>beforeTask</b> <code>{function}</code>:
        <p>Called before each Task is called. Use this to override any globals you want to exist during the execution of the task:</p>
<pre><code class="language-js">new Zone({
    beforeTask: function(){
      ...
      <li>
        <b>ended</b> <code>{function}</code>:
        <p>Called when the Zone has ended and is about to exit (it's Promise will resolve).</p>

      </li>
    
      <li>
        <b>hooks</b> <code>{Array&lt;string&gt;}</code>:
        <p><strong>hooks</strong> allows you to specify custom hooks that your plugin calls. This is mostly to communicate between plugins that inherit each other.</p>
<pre><code class="language-js">var barZone = {
    created: function(){
...
zone.run(function() { ... });
      ...
      <li>
        <b>plugins</b> <code>{Array&lt;<a href="can-zone.ZoneSpec.html" title="A ZoneSpec is the way you tap into the lifecycle hooks of a Zone. The hooks are described below.
Using these hooks you can do things like create timers and override global variables that will change the shape of code that runs within the Zone.">ZoneSpec</a>|<a href="can-zone.makeZoneSpec.html" title="A function that returns a can-zone.ZoneSpec object. This can be used any place where a can-zone.ZoneSpec is accepted.">makeZoneSpec</a>(<a href="can-zone.prototype.data.html" title="">data</a>)&gt;}</code>:
        <p>Allows specifying nested <a href="can-zone.ZoneSpec.html" title="A ZoneSpec is the way you tap into the lifecycle hooks of a Zone. The hooks are described below.
Using these hooks you can do things like create timers and override global variables that will change the shape of code that runs within the Zone.">ZoneSpecs</a> that the current depends on. This allows creating rich plugins that depend on other plugins (ZoneSpecs). You can imagine having a bunch of tiny plugins that do one thing and then composing them together into one meta-plugin that is more end-user friendly.</p>
<p>Similar to the <a href="can-zone.html" title="A library that tracks asynchronous activity and lets you know when it has completed. Useful when you need to call a function and wait for all async behavior to complete, such as when performing server-side rendering.">Zone</a> constructor you can either specify <a href="can-zone.ZoneSpec.html" title="A ZoneSpec is the way you tap into the lifecycle hooks of a Zone. The hooks are described below.
...
<pre><code class="language-js">var specOne = {
    created: function(){
