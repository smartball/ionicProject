...
<!--####################################################################
	...
	INSTEAD CHANGE:
	source: node_modules/can-stache/docs/tags/escaped.md
	@function can-stache.tags.escaped
######################################################################## -->
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>CanJS - {{expression}}</title>
...
call.">
					...
					<li class="
						...
						<a class="typedef"
							...
							title="Event bindings and some helpers like can-stache.helpers.each provide special values that start with % to prevent potential collisions with
other values. Special values should not be confused with template variables like *self">
							%special
						</a>
					...
					<li class="
           						...
           						">
						<a class="typedef"
							href="can-stache/keys/variable/self.html"
							title="Used to reference the current template and recursively render it">
							*self
						</a>
...
<div class="breadcrumb">
	...
	<li><a href="can-stache.tags.escaped.html">{{expression}}</a></li>
  ...
  <section class="title">
	<div class="page-type">
		<h1>{{expression}}</h1>
			<ul class="title-social">
				...
				<li>
					<a class="button-link" href="//github.com/canjs/can-stache/edit/master/docs/tags/escaped.md">Edit on GitHub</a>
				</li>
    ...
    <p>Insert the value of the expression into the output of the template.</p>
...
<div class="signature">
  ...
  <p>Gets the value of <code>EXPRESSION</code> and inserts the result into the output of the
template.</p>
<p>If the expression is clearly of a particular expression type like: <code>{{myHelper arg}}</code> or
<code>{{myMethod(arg)}}</code>, that expression's rules will be followed.</p>
<p>An ambiguous expression type like <code>{{keyOrHelper}}</code> will first treat <code>keyOrHelper</code>
as a <a href="can-stache/expressions/key-lookup.html" title="">KeyLookup Expression</a> and if there is no value in the scope of
<code>keyOrHelper</code>, it will be treated as a <a href="can-stache/expressions/helper.html" title="">Helper Expression</a>.</p>
  ...
  <div class="parameters">
    <h3 class="parameters-title">Parameters</h3>
      <ol>
          ...
          <li><b>expression</b> <code>{<a href="can-stache/expressions/literal.html" title="Specify a JavaScript primitive type.">Literal Expression</a>|<a href="can-stache/expressions/key-lookup.html" title="">KeyLookup Expression</a>|<a href="can-stache/expressions/call.html" title="">Call Expression</a>|<a href="can-stache/expressions/helper.html" title="">Helper Expression</a>}</code>: <p>The <code>expression</code> can be:</p>
<ul>
<li><a href="can-stache/expressions/literal.html" title="Specify a JavaScript primitive type.">Literal Expression</a> - <code>{{5}}</code> - Inserts a string representation of the literal.</li>
<li><a href="can-stache/expressions/key-lookup.html" title="">KeyLookup Expression</a> - <code>{{key}}</code> - Looks up the value of <code>key</code> in the <a href="can-view-scope.html" title="Create a lookup node for keys.">can-view-scope</a>.</li>
<li><a href="can-stache/expressions/call.html" title="">Call Expression</a> - <code>{{method()}}</code> - Calls <code>method</code> in the <a href="can-view-scope.html" title="Create a lookup node for keys.">can-view-scope</a>.</li>
<li><a href="can-stache/expressions/helper.html" title="">Helper Expression</a> - <code>{{helper arg}}</code> - Calls <code>helper</code> in the <a href="can-view-scope.Options.html" title="">Options</a> and passes it a <a href="can-stache.helperOptions.html" title="The options argument passed to a helper function when called by a helper.">helperOptions</a>.</li>
</ul>
</li>
    ...
    <section class="body">
    <h2>Use</h2>
<p>The following breaks down the behavior of <code>{{expression}}</code>.  It groups
the behavior of <a href="can-stache/expressions/key-lookup.html" title="">KeyLookup Expression</a> and <a href="can-stache/expressions/call.html" title="">Call Expression</a>s
because their behavior works the same way.  It then details how <a href="can-stache/expressions/helper.html" title="">Helper Expression</a>s
work.</p>
<h3>Key and Call Expressions</h3>
<p><code>{{key}}</code> insert data into the template. It most commonly references
values within the current context. For example:</p>
<p>Rendering:</p>
<pre><code>&lt;h1&gt;{{name}}&lt;/h1&gt;
</code></pre>
<p>With:</p>
<pre><code>{name: &quot;Austin&quot;}
</code></pre>
<p>Results in:</p>
<pre><code>&lt;h1&gt;Austin&lt;/h1&gt;
</code></pre>
<p>If the key value is a String or Number, it is inserted into the template.
If it is <code>null</code> or <code>undefined</code>, nothing is added to the template.</p>
<h3>Nested Properties</h3>
<p>Stache supports nested paths, making it possible to
look up properties nested deep inside the current context. For example:</p>
<p>Rendering:</p>
<pre><code>&lt;h1&gt;{{book.author}}&lt;/h1&gt;
</code></pre>
<p>With:</p>
<pre><code>{
  book: {
...
<p>Results in:</p>
<pre><code>&lt;h1&gt;Ernest Hemingway&lt;/h1&gt;
</code></pre>
<h3>Looking up values in parent contexts</h3>
<p>Sections and block helpers can create their own contexts. If a key's value
is not found in the current context, it will look up the key's value
in parent contexts. For example:</p>
<p>Rendering:</p>
<pre><code>{{#chapters}}
   &lt;li&gt;{{title}} - {{name}}&lt;/li&gt;
{{chapters}}
...
<p>Results in:</p>
<pre><code>&lt;li&gt;The Book of Bitovi - Breakdown&lt;/li&gt;
</code></pre>
<h2>Helper expressions</h2>
<p>The <code>{{helper}}</code> syntax is used to call out to stache <a href="can-stache.helper.html" title="A helper function passed to can-stache.registerHelper.
Given the arguments, returns the content that should be shown in the DOM
or a function that will be called on the DOM element the helper was
called on.">helper functions</a> functions
that may contain more complex functionality. <code>helper</code> is a <a href="can-stache.key.html" title="A named reference to a value in the scope or helper scope in a template.">key</a> that must match either:</p>
<ul>
<li>a <a href="can-stache.registerHelper.html" title="Register a helper.">registered helper function</a>, or</li>
<li>a function in the current or parent <a href="can-stache.scopeAndContext.html" title="">contexts</a></li>
</ul>
<p>The following example shows both cases.</p>
<p>The Template:</p>
<pre><code>&lt;p&gt;{{greeting}} {{user}}&lt;/p&gt;
</code></pre>
<p>Rendered with data:</p>
<pre><code>{
  user: function(){ return &quot;Justin&quot; }
}
</code></pre>
<p>And a with a registered helper like:</p>
<pre><code>stache.registerHelper('greeting', function(){
  return &quot;Hello&quot;
});
</code></pre>
<p>Results in:</p>
...
<h3>Arguments</h3>
<p>Arguments can be passed from the template to helper function by
listing space seperated strings, numbers or other <a href="can-stache.key.html" title="A named reference to a value in the scope or helper scope in a template.">keys</a> after the
<code>helper</code> name.  For example:</p>
<p>The template:</p>
...
<pre><code>stache.registerHelper('madLib',
...
<p>If an argument <code>key</code> value is a <a href="can-map.html" title="Create observable objects.">can-map</a> property, the Observe's
property is converted to a getter/setter <a href="can-compute.computed.html" title="A derived value from other computes and observable maps.">compute</a>. For example:</p>
<p>The template:</p>
<pre><code>&lt;p&gt;What! My name is: {{mr user.name}}&lt;/p&gt;
</code></pre>
<p>Rendered with:</p>
<pre><code>{user: new Map({name: &quot;Slim Shady&quot;})}
</code></pre>
<p>Needs the helper to check if name is a function or not:</p>
<pre><code>stache.registerHelper('mr',function(name){
  return &quot;Mr. &quot;+ (typeof name === &quot;function&quot; ?
                  name():
                  name)
})
</code></pre>
<p>This behavior enables two way binding helpers and is explained in more detail
on the <a href="can-stache.helper.html" title="A helper function passed to can-stache.registerHelper.
Given the arguments, returns the content that should be shown in the DOM
or a function that will be called on the DOM element the helper was
called on.">helper functions</a> docs.</p>
<h3>Hash</h3>
<p>If enumerated arguments isn't an appropriate way to configure the behavior
of a helper, it's possible to pass a hash of key-value pairs to the
<a href="can-stache.helperOptions.html" title="The options argument passed to a helper function when called by a helper.">helper option argument</a>'s
hash object.  Properties and values are specified
as <code>hashProperty=hashValue</code>.  For example:</p>
<p>The template:</p>
<pre><code>&lt;p&gt;My {{excuse who=pet how=&quot;shreded&quot;}}&lt;/p&gt;
</code></pre>
<p>`
And the helper:</p>
<pre><code>stache.registerHelper(&quot;excuse&quot;,function(options){
  return [&quot;My&quot;,
...
<p>Results in:</p>
<pre><code>&lt;p&gt;My cat shareded my homework&lt;/p&gt;
</code></pre>
<h3>Returning an element callback function</h3>
<p>If a helper returns a function, that function is called back after
the template has been rendered into DOM elements. This can
be used to create stache tags that have rich behavior. Read about it
on the <a href="can-stache.helper.html" title="A helper function passed to can-stache.registerHelper.
Given the arguments, returns the content that should be shown in the DOM
or a function that will be called on the DOM element the helper was
called on.">helper function</a> page.</p>
